...
const scheduleEvent = (date, prayer, event) => {
  ...
  if (adjustedDate < now) {
    ...
    if (diffSeconds <= 60) {
      console.warn(
        `[Scheduler] Missed ${prayer} ${event} by ${diffSeconds.toFixed(1)}s, executing catch-up.`,
      );
      automationService.triggerEvent(prayer, event);
    } else {
      console.warn(
        `[Scheduler] Adjusted time for ${prayer} ${event} is in the past (${adjustedDate.toFormat("HH:mm:ss")}), skipping.`,
      );
    }
    return;
  }
  ...
  if (job) {
    ...
    jobs.push(job);
    console.log(
      `[Scheduler] Scheduled ${prayer} ${event} at ${adjustedDate.toFormat("HH:mm:ss")} (Original: ${date.toFormat("HH:mm:ss")}, Lead: ${maxLeadTime}ms)`,
    );
  }
};
...
const scheduleMaintenanceJobs = () => {
  ...
  const staleAction = async () => {
    ...
    try {
      ...
      if (cache && cache.meta && cache.meta.lastFetched) {
        ...
        if (days > staleDays) {
          console.log(
            `[Maintenance] Data stale (${days.toFixed(1)} days). Refreshing...`,
          );
  ...
  const boundaryAction = async () => {
    ...
    try {
      ...
      if (diff >= 0 && diff <= 7) {
        ...
        const nextJan1 = DateTime.fromObject({
          year: nextYear,
          month: 1,
          day: 1,
        });
        ...
        if (!cache.data || !cache.data[nextKey]) {
          console.log(
            `[Maintenance] Approaching end of year. Fetching ${nextYear}...`,
          );
  ...
  // Schedule Jobs
  const staleJob = schedule.scheduleJob(
    jobConstants.JOB_STALE_CHECK,
    "0 3 * * 0",
    schedulerWrap(staleAction),
  );
  if (staleJob) {
  ...
  const boundaryJob = schedule.scheduleJob(
    jobConstants.JOB_YEAR_BOUNDARY,
    "0 4 * * *",
    schedulerWrap(boundaryAction),
  );
  if (boundaryJob) {
  ...
  const healthJob = schedule.scheduleJob(
    jobConstants.JOB_HEALTH_CHECK,
    "0 0 * * *",
    schedulerWrap(healthAction),
  );
  if (healthJob) {
  ...
  const assetMaintenanceJob = schedule.scheduleJob(
    jobConstants.JOB_AUDIO_ASSETS,
    "30 3 * * *",
    schedulerWrap(assetAction),
  );
  ...
  const midnightJob = schedule.scheduleJob(
    jobConstants.JOB_MIDNIGHT_REFRESH,
    "0 0 * * *",
    schedulerWrap(midnightAction),
  );
  if (midnightJob) {
...
const initScheduler = async () => {
  ...
  try {
    ...
    for (const prayer of prayerNames) {
      ...
      const start = DateTime.fromISO(startISO).setZone(
      ...
      if (prayer !== "sunrise") {
        ...
        // 2. Calculated (Fallback or Forced Override)
          ...
          const calculatedIso = calculateIqamah(
            startISO,
            prayerConfig,
            config.location.timezone,
          );
          iqamah = DateTime.fromISO(calculatedIso).setZone(
            config.location.timezone,
          );
        }
      }
    ...
    console.log(
      `[Scheduler] Initialisation complete. ${jobs.length} jobs scheduled.`,
    );
  } catch (error) {
...
const getJobs = () => {
  ...
  const formatJob = (j) => {
    ...
    try {
      ...
      console.error(
        `[Scheduler] Error calculating next invocation for ${j.jobName}:`,
        e,
      );
    }
    ...
    if (next) {
      try {
        ...
        // nosemgrep: unsafe-formatstring -- j.jobName is an internal scheduler constant, not user HTTP input
        console.error(
          `[Scheduler] Date conversion failed for ${j.jobName}:`,
          err,
        );
      }
    }
  ...
  const maintenance = jobs
    .filter((j) => j.category === "maintenance")
    .map(formatJob);
  const automation = jobs
    .filter((j) => j.category === "automation")
    .map(formatJob);
...
const runJob = async (jobName) => {
  ...
  try {
    ...
    console.error(
      `[Scheduler] Manual execution of "${jobName}" failed:`,
      error,
    );
    return {
      success: false,
