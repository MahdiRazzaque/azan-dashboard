...
const _loadFromCache = async () => {
  try {
    ...
    if (_isCacheValid()) {
      voices = data.voices;
      console.log(
        `[VoiceService] Loaded ${voices.length} voices from disk cache (Last updated: ${lastFetched.toISOString()}).`,
      );
      return voices;
    } else {
...
const refreshVoices = async () => {
  ...
  try {
    ...
    if (response.data && Array.isArray(response.data)) {
      ...
      console.log(
        `[VoiceService] Successfully cached ${voices.length} voices.`,
      );
    }
    return voices;
  } catch (error) {
    console.error(
      "[VoiceService] Failed to fetch voices from Python service:",
      error.message,
    );
