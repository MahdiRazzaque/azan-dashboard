...
class MigrationService {
  ...
  setOutputSecretKeysResolver(resolver) {
    if (typeof resolver !== "function") {
      throw new TypeError(
        "[MigrationService] setOutputSecretKeysResolver() expects a function.",
      );
    }
  ...
  migrateEnvSecrets(config) {
    ...
    if (!this._outputSecretKeysResolver) {
      throw new Error(
        "[MigrationService] Output secret keys resolver not wired. Call setOutputSecretKeysResolver() before migrateEnvSecrets().",
      );
    }
    ...
    outputSecrets.forEach(({ strategyId, key }) => {
      ...
      if (envValue) {
        ...
        if (!config.automation.outputs[strategyId]) {
          config.automation.outputs[strategyId] = {
            enabled: false,
            params: {},
          };
        }
    ...
    for (const provider of providers) {
      const sensitiveKeys =
        provider.parameters?.filter((p) => p.sensitive).map((p) => p.key) || [];
  ...
  migrateV2toV3(config) {
    ...
    if (v3Config.calculation && v3Config.sources && v3Config.sources.primary) {
      ...
      if (primary.type === "aladhan") {
        ...
        primary.latitudeAdjustmentMethod =
          v3Config.calculation.latitudeAdjustmentMethod;
