...
function init() {
  ...
  try {
    ...
    console.warn(
      "[HealthCheck] Failed to dynamically load strategies into cache",
    );
  }
}
...
async function checkSource(target) {
  ...
  if (target === "backup" && source.enabled === false)
