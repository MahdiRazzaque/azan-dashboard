...
class AssetMigrationService {
    ...
    async migrateAll() {
        ...
        try {
            const directories = [
                { audio: path.join(AUDIO_ROOT, 'custom'), meta: path.join(META_ROOT, 'custom') },
                { audio: path.join(AUDIO_ROOT, 'cache'), meta: path.join(META_ROOT, 'cache') }
            ];
            ...
            for (const dirSet of directories) {
                ...
                for (let i = 0; i < files.length; i += BATCH_SIZE) {
                    ...
                    console.log(`[Migration] Processing batch ${(i / BATCH_SIZE) + 1}/${Math.ceil(files.length / BATCH_SIZE)} in ${path.basename(dirSet.audio)}...`);
                    
                    const results = await Promise.all(batch.map(file => this._migrateFile(file, dirSet)));
            ...
            console.log(`[Migration] Completed in ${duration}s. Processed: ${totalProcessed}, Updated: ${totalUpdated}`);
        } catch (error) {
            console.error('[Migration] Critical failure during asset migration:', error.message);
        }
    }
