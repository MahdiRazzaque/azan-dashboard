...
class ConfigurationWorkflowService {
  ...
  async executeUpdate(newConfig) {
    ...
    const sourcesChanged =
      JSON.stringify(previousConfig.sources) !==
      JSON.stringify(newConfig.sources);
    const locationChanged =
      JSON.stringify(previousConfig.location) !==
      JSON.stringify(newConfig.location);
    ...
    if (requiresRefresh) {
      sseService.broadcast({
        type: "PROCESS_UPDATE",
        payload: { label: "Validating Configuration..." },
      });
    ...
    if (requiresRefresh) {
      sseService.broadcast({
        type: "PROCESS_UPDATE",
        payload: { label: "Refreshing Prayer Data..." },
      });
    ...
    sseService.broadcast({
      type: "PROCESS_UPDATE",
      payload: { label: "Generating Audio Assets..." },
    });
    ...
    sseService.broadcast({
      type: "PROCESS_UPDATE",
      payload: { label: "Configuration Saved" },
    });
  ...
  async _collectWarnings(syncResult, health, finalConfig) {
    ...
    if (finalConfig.automation && finalConfig.automation.triggers) {
      Object.entries(finalConfig.automation.triggers).forEach(
        ([prayer, triggers]) => {
          Object.entries(triggers).forEach(([type, trigger]) => {
            ...
            (trigger.targets || []).forEach((targetId) => {
              ...
              if (!outputConfig || !outputConfig.enabled) {
                warnings.push(
                  `${niceName}: ${strategy.label} output is disabled in settings`,
                );
              } else if (strategyHealth && !strategyHealth.healthy) {
                warnings.push(
                  `${niceName}: ${strategy.label} output is offline (${strategyHealth.message || "unreachable"})`,
                );
              }
