...
jest.mock("@utils/normalizeSource", () => {
  return jest.fn((source) => {
    ...
    return {
      ...
      filePath:
        source.filePath ||
        `/resolved/public/audio/${source.path || "test.mp3"}`,
...
describe("VoiceMonkeyOutput", () => {
  ...
  describe("_executeFromFile", () => {
    ...
    it("should skip if baseUrl is not HTTPS for relative URL", async () => {
      ConfigService.get.mockReturnValue({
        automation: {
          ...
          outputs: { voicemonkey: { params: { token: "t", device: "d" } } },
        },
      });
      await output._executeFromFile(filePayload, {});
    ...
    it("should skip if token or device missing", async () => {
      ConfigService.get.mockReturnValue({
        automation: { baseUrl: "https://ok.com" },
      });
    ...
    it("should use test prefix when isTest is true", async () => {
      ...
      expect(spy).toHaveBeenCalledWith(
        expect.stringContaining("[Test Output: VoiceMonkey]"),
      );
      spy.mockRestore();
    });
  });
  ...
  describe("_executeFromUrl", () => {
    ...
    it("should throw if URL is not HTTPS", async () => {
      ...
      await expect(output._executeFromUrl(payload, {})).rejects.toThrow(
  ...
  describe("execute (integration via BaseOutput template method)", () => {
    ...
    it("should dispatch URL source to _executeFromUrl", async () => {
      ...
      await output.execute(
        {
          source: { url: "https://cdn.example.com/audio.mp3" },
          params: { token: "t1", device: "d1" },
        },
        {},
      );
  ...
  describe("_sendToApi", () => {
    it("should use token from payload params over config", async () => {
      ...
      axios.get.mockResolvedValue({ data: { success: true } });
      await output._sendToApi(
        "https://test.com/audio.mp3",
        { params: { token: "p-token", device: "p-device" } },
        "[Test]",
      );
      expect(axios.get).toHaveBeenCalledWith(
        expect.anything(),
  ...
  describe("healthCheck", () => {
    it("should return healthy if API returns success", async () => {
      ConfigService.get.mockReturnValue({
        automation: {
          ...
          outputs: { voicemonkey: { params: { token: "t" } } },
        },
      });
      axios.get.mockResolvedValue({ data: { success: true } });
    ...
    it("should return offline if baseUrl is not HTTPS", async () => {
      ConfigService.get.mockReturnValue({
        automation: { baseUrl: "http://insecure.com" },
      });
      const result = await output.healthCheck();
    ...
    it("should handle API failure response", async () => {
      ...
      axios.get.mockResolvedValue({
        data: { success: false, error: "Some API Error" },
      });
      const result = await output.healthCheck();
      expect(result.healthy).toBe(false);
    ...
    it("should use requestedParams token over config", async () => {
      ...
      expect(axios.get).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({
          params: expect.objectContaining({ token: "override-token" }),
        }),
      );
    });
  });
  ...
  describe("verifyCredentials", () => {
    ...
    it("should handle API failure response", async () => {
      ...
      await expect(
        output.verifyCredentials({ token: "t", device: "d" }),
  ...
  describe("validateTrigger", () => {
    ...
    it("should return no warnings if file not found", () => {
      ...
      const result = output.validateTrigger(
        { type: "file", path: "missing.mp3" },
        context,
      );
        ...
        audioFiles: [
          {
            path: "p.mp3",
            compatibility: {
              voicemonkey: { valid: false, issues: ["Too long"] },
            },
          },
        ],
      };
      const result = output.validateTrigger(
        { type: "file", path: "p.mp3" },
        context,
      );
    ...
    it("should handle missing issues array", () => {
      const context = {
        ...baseContext,
        audioFiles: [
          { path: "p.mp3", compatibility: { voicemonkey: { valid: false } } },
        ],
      };
      const result = output.validateTrigger(
        { type: "file", path: "p.mp3" },
    ...
    it("should skip if compatibility is not explicitly false", () => {
      const context = {
        ...baseContext,
        audioFiles: [
          { path: "p.mp3", compatibility: { voicemonkey: { valid: true } } },
        ],
      };
      const result = output.validateTrigger(
        { type: "file", path: "p.mp3" },
        ...
        audioFiles: [
          {
            ...
            compatibility: {
              voicemonkey: { valid: false, issues: ["Bad format"] },
            },
          },
        ],
      };
  ...
  describe("validateAsset", () => {
    ...
    it("should fallback to manual check if mimeType is missing", async () => {
      expect(
        (await output.validateAsset("p.mp3", { format: "mp3" })).valid,
      ).toBe(true);
      expect(
        (await output.validateAsset("p.wav", { format: "wav" })).valid,
      ).toBe(true);
      expect(
        (await output.validateAsset("p.ogg", { format: "ogg" })).valid,
      ).toBe(true);
      expect(
        (await output.validateAsset("p.aac", { format: "aac" })).valid,
      ).toBe(true);
      expect(
        (await output.validateAsset("p.x", { format: "unknown" })).valid,
      ).toBe(false);
    });
