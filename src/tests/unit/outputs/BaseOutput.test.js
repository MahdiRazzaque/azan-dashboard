...
describe('BaseOutput', () => {
    ...
    describe('execute — Template Method', () => {
        ...
        it('should normalize raw source before dispatching', async () => {
            ...
            expect(output.lastCall.payload.source.filePath).toContain('custom/test.mp3');
            expect(output.lastCall.payload.source.url).toBe('/public/audio/custom/test.mp3');
        });
    ...
    describe('execute — sidecar compatibility', () => {
        ...
        it('should use output metadata id to look up compatibility', async () => {
            class AlexaOutput extends BaseOutput {
                static getMetadata() {
                    return { id: 'alexa', label: 'Alexa', supportedSourceTypes: ['file'], params: [] };
                }
                async _executeFromFile(payload) { this.lastCall = { hook: 'file', payload }; }
            }
