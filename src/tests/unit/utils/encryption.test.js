...
describe("Encryption Utils", () => {
  ...
  describe("encrypt/decrypt", () => {
    ...
    it("should throw error for invalid key", async () => {
      const ciphertext = await encrypt(plaintext, secretKey);
      await expect(
        decrypt(ciphertext, "wrong-key-32-chars-long-!!!!!!!"),
      ).rejects.toThrow();
    });
    ...
    it("should reject a truncated auth tag", async () => {
      ...
      await expect(
        decrypt(`${iv}:${truncatedAuthTag}:${encrypted}`, secretKey),
      ).rejects.toThrow();
    });
    ...
    it("should throw error for invalid ciphertext format (wrong number of parts)", async () => {
      await expect(decrypt("part1:part2", secretKey)).rejects.toThrow(
        "Invalid ciphertext format",
      );
      await expect(
        decrypt("part1:part2:part3:part4", secretKey),
    ...
    it("should throw error if ENCRYPTION_SALT is missing", async () => {
      ...
      try {
        await expect(encrypt(plaintext, secretKey)).rejects.toThrow(
