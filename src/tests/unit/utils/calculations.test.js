...
describe("Calculation Utils", () => {
  ...
  describe("calculateIqamah", () => {
    it("should throw error for invalid inputs", () => {
      ...
      expect(() => calculateIqamah("2023-01-01", {}, null)).toThrow(
        "Invalid arguments",
      );
    });

    it("should throw error for invalid ISO format", () => {
      expect(() => calculateIqamah("invalid-date", {}, timezone)).toThrow(
        "Invalid prayerStartISO format",
      );
    });

    it("should not round if roundTo is 0", () => {
      const prayerStart = DateTime.fromObject(
        { hour: 12, minute: 7 },
        { zone: timezone },
      ).toISO();
