...
describe("normalizeSource", () => {
  ...
  describe("path traversal protection", () => {
    it("should reject path with ../ traversal", () => {
      expect(() => normalizeSource({ path: "../../../etc/passwd" })).toThrow(
    ...
    it("should reject path that resolves outside AUDIO_ROOT", () => {
      expect(() =>
        normalizeSource({ path: "custom/../../outside.mp3" }),
      ).toThrow("Path traversal detected");
    });

    it("should reject sibling prefix escapes like ../audio-evil/test.mp3", () => {
      expect(() => normalizeSource({ path: "../audio-evil/test.mp3" })).toThrow(
        "Path traversal detected",
      );
    });
  });
  ...
  describe("URL security", () => {
    it("should reject file:// protocol URLs", () => {
      expect(() => normalizeSource({ url: "file:///etc/passwd" })).toThrow(
    ...
    it("should reject data: URLs", () => {
      expect(() =>
        normalizeSource({ url: "data:audio/mp3;base64,abc" }),
      ).toThrow("Only http and https URLs are allowed");
    });

    it("should reject ftp: URLs", () => {
      expect(() => normalizeSource({ url: "ftp://server/file.mp3" })).toThrow(
        "Only http and https URLs are allowed",
      );
    });
  });
