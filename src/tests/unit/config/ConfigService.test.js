...
describe("ConfigService", () => {
  ...
  it("should throw error if get() called before init()", () => {
    ...
    expect(() => configService.get()).toThrow(
      "ConfigService not initialised. Call init() first.",
    );
  });
  ...
  it("should apply environment variable overrides", async () => {
    ...
    expect(config.automation.outputs.voicemonkey.params.device).toBe(
      "env-device",
    );
  ...
  it("should merge arrays by overwriting", async () => {
    ...
    await configService.update(updateData);
    const config = configService.get();
    expect(config.automation.triggers.fajr.preAdhan.targets).toEqual([
      "voiceMonkey",
    ]);
  });
  ...
  it("should log warning if local config fails to load with non-ENOENT error", async () => {
    ...
    jest
      .spyOn(fs, "access")
      .mockRejectedValue({ code: "EACCES", message: "Permission denied" });

    await configService.reload();

    expect(spy).toHaveBeenCalledWith(
      expect.stringContaining("Warning: Failed to load local config"),
      expect.anything(),
    );
    spy.mockRestore();
  ...
  it("should throw error if local config fails to load with non-ENOENT error in update", async () => {
    ...
    await expect(configService.update({})).rejects.toMatchObject({
      code: "EACCES",
    });
  ...
  it("should migrate V1 local config to V4 on reload", async () => {
    const v1Local = {
      automation: {
        voiceMonkey: {
          enabled: true,
  ...
  it("should format ZodError in reload", async () => {
    ...
    await expect(configService.reload()).rejects.toThrow(
      "Configuration Validation Failed: location.timezone:",
    );
  });
  ...
  it("should handle atomic save failure cleanup", async () => {
    ...
    await expect(
      configService.update({ data: { staleCheckDays: 1 } }),
    ).rejects.toThrow("Atomic configuration save failed");
  ...
  it("should validate source-specific configuration", async () => {
    const invalidSource = {
      sources: { primary: { type: "aladhan", method: "invalid" } },
    };
    await expect(configService.update(invalidSource)).rejects.toThrow(
  ...
  it("should clean source data when type changes", async () => {
    ...
    const update = {
      sources: {
        primary: {
          type: "mymasjid",
          masjidId: "00000000-0000-0000-0000-000000000000",
        },
      },
    };
  ...
  it("should apply provider secrets from env", async () => {
    ...
    const config = {
      sources: { primary: { type: "test-prov" } },
      automation: {},
    };

    const { ConfigService } = require("../../../config/ConfigService");
    const myConfigService = new ConfigService();
    myConfigService.setOutputSecretKeysResolver(() =>
  ...
  it("should process sensitive fields for sources", async () => {
    ...
    jest
      .spyOn(ProviderFactory, "getProviderClass")
      .mockReturnValue(mockProviderClass);
