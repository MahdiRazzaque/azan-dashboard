...
describe('AladhanProvider', () => {
    ...
    describe('_doFetch edge cases', () => {
        it('should handle schema validation failure with details', async () => {
            global.fetch = jest.fn().mockResolvedValue({
                ok: true,
                json: () => Promise.resolve({ code: 200, status: 'OK', data: { "1": [{ timings: {} }] } })
            });
            await expect(provider.getAnnualTimes(2024)).rejects.toThrow('Aladhan Schema Validation Failed');
        });
        ...
        it('should handle missing time strings', async () => {
            const mockData = {
                ...
                data: {
                    "1": [{
                        timings: { 
                            Fajr: '', Sunrise: '', Dhuhr: '', Asr: '', Maghrib: '', Isha: '', Imsak: '', Midnight: '',
                            Sunset: ''
                        },
                        date: { gregorian: { date: '01-01-2024', day: '01', month: { number: 1 }, year: '2024' } },
        ...
        it('should handle fetch network error', async () => {
            ...
            await expect(provider.getAnnualTimes(2024)).rejects.toThrow('Failed to connect to Aladhan API: Network Fail');
        });
    });

    describe('Helper methods', () => {
        it('should resolve calculation method by name', async () => {
            const p = new AladhanProvider({ method: 'Muslim World League' }, globalConfig);
            global.fetch = jest.fn().mockResolvedValue({ ok: false, status: 400 });
            try { await p.getAnnualTimes(2024); } catch (e) {}
        ...
        it('should resolve latitude adjustment by name', async () => {
            const p = new AladhanProvider({ latitudeAdjustmentMethod: 'Angle Based' }, globalConfig);
            global.fetch = jest.fn().mockResolvedValue({ ok: false, status: 400 });
            try { await p.getAnnualTimes(2024); } catch (e) {}
    ...
    describe('Helper methods extra', () => {
        it('should use numeric IDs if provided', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('method=3'));
            expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('school=1'));
            expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('latitudeAdjustmentMethod=1'));
            expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining('midnightMode=1'));
        });
        ...
        it('should handle validation error without issues array', async () => {
            ...
            const { AladhanAnnualResponseSchema } = require('@providers/AladhanProvider');
