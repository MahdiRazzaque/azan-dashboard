...
describe("MyMasjidProvider", () => {
      ...
      salahTimings: [
        {
          ...
          month: 1,
          fajr: [
            { salahName: "Fajr", salahTime: "05:00", iqamahTime: "05:30" },
          ],
          zuhr: [
            { salahName: "Dhuhr", salahTime: "12:30", iqamahTime: "13:00" },
          ],
          asr: [{ salahName: "Asr", salahTime: "15:30", iqamahTime: "16:00" }],
          maghrib: [
            { salahName: "Maghrib", salahTime: "17:00", iqamahTime: "17:15" },
          ],
          isha: [
            { salahName: "Isha", salahTime: "19:00", iqamahTime: "20:00" },
          ],
          shouruq: [
            { salahName: "Sunrise", salahTime: "07:00", iqamahTime: null },
          ],
        },
      ],
    },
  ...
  it("should fetch and parse nested MyMasjid data", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(
      expect.stringContaining("GuidId=test-guid"),
    );
  ...
  describe("_doFetch edge cases", () => {
    it("should throw if masjidId is missing", async () => {
      ...
      await expect(p.getAnnualTimes(2024)).rejects.toThrow(
        "Masjid ID is required",
      );
    });
    ...
    it("should handle schema validation failure", async () => {
      global.fetch.mockResolvedValue({
        ok: true,
        json: () =>
          Promise.resolve({ model: { salahTimings: [{ day: "invalid" }] } }),
      });
      await expect(provider.getAnnualTimes(2024)).rejects.toThrow(
        "MyMasjid Schema Validation Failed",
      );
    });
