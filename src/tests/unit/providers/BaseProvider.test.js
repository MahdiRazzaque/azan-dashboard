...
describe("BaseProvider", () => {
  ...
  it("should throw error if getAnnualTimes is not implemented", async () => {
    ...
    await expect(base.getAnnualTimes(2024)).rejects.toThrow(
      "Method getAnnualTimes() must be implemented",
    );
  });

  it("should throw error if healthCheck is not implemented", async () => {
    const base = new BaseProvider();
    await expect(base.healthCheck()).rejects.toThrow(
      "Method healthCheck() must be implemented",
    );
  });
