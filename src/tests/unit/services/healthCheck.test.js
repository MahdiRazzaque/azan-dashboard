...
describe("HealthCheck Service", () => {
  ...
  beforeEach(() => {
    ...
    mockLocalStrategy = {
      ...
      healthCheck: jest
        .fn()
        .mockResolvedValue({ healthy: true, message: "Ready" }),
    };
    mockVMStrategy = {
      constructor: {
        getMetadata: () => ({ id: "voicemonkey", hidden: false }),
      },
      healthCheck: jest
        .fn()
        .mockResolvedValue({ healthy: true, message: "Online" }),
    };
  ...
  it("should handle checkSource using provider healthCheck", async () => {
    const mockProvider = {
      healthCheck: jest
        .fn()
        .mockResolvedValue({ healthy: true, message: "Source OK" }),
    };
  ...
  it("should filter params passed to strategies based on requiredForHealth", async () => {
    ...
    expect(mockLocalStrategy.healthCheck).toHaveBeenCalledWith({
      audioPlayer: "mpg123",
    });
