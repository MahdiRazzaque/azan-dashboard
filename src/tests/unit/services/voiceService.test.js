...
describe('VoiceService (Async Refactor)', () => {
    ...
    it('should use custom python service URL', async () => {
        ...
        expect(axios.get).toHaveBeenCalledWith(expect.stringContaining('http://custom:9000'), expect.anything());
    });
    ...
    it('should use default python service URL if not configured', async () => {
        ...
        await service.refreshVoices();
        expect(axios.get).toHaveBeenCalledWith(expect.stringContaining('http://localhost:8000'), expect.anything());
    });
