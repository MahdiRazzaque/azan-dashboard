...
describe('StorageService', () => {
    ...
    describe('getDirSize', () => {
        it('should recursively calculate directory size', async () => {
            ...
            fs.promises.readdir.mockResolvedValueOnce(['file1.mp3', 'subdir'])
                         .mockResolvedValueOnce(['file2.mp3']);
            
            fs.promises.lstat.mockResolvedValueOnce({ isDirectory: () => false, isSymbolicLink: () => false, size: 1000 }) // file1
                       .mockResolvedValueOnce({ isDirectory: () => true, isSymbolicLink: () => false })            // subdir
                       .mockResolvedValueOnce({ isDirectory: () => false, isSymbolicLink: () => false, size: 2000 }); // file2
        ...
        it('should prevent recursion (circular symlinks or same path)', async () => {
            ...
            fs.promises.lstat.mockResolvedValue({ isDirectory: () => true, isSymbolicLink: () => false });
            
            // To prevent actual infinite recursion in test if visitedPaths fails
            // we mock readdir to return empty after some calls
            fs.promises.readdir.mockResolvedValueOnce(['subdir'])
    ...
    describe('getUsage', () => {
        it('should get breakdown', async () => {
             const spy = jest.spyOn(storageService, 'getDirSize').mockResolvedValue(50);
    ...
    describe('checkQuota', () => {
        it('should use default limit if config missing', async () => {
             ...
             const spy = jest.spyOn(storageService, 'getUsage').mockResolvedValue({ total: 0 });
