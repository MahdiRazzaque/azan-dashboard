...
describe('migrationService', () => {
    ...
    describe('migrateEnvSecrets', () => {
        it('should migrate output secrets from env', () => {
            const config = { automation: { outputs: { voicemonkey: { params: {} } } } };
            process.env.VOICEMONKEY_TOKEN = 'env-token';
            
            OutputFactory.getSecretRequirementKeys.mockReturnValue([{ strategyId: 'voicemonkey', key: 'token' }]);
            ProviderFactory.getRegisteredProviders.mockReturnValue([]);

            const result = migrationService.migrateEnvSecrets(config);
            
            expect(result.changed).toBe(true);
            expect(result.config.automation.outputs.voicemonkey.params.token).toBe('env-token');
        ...
        it('should create missing params block during output migration', () => {
            const config = { automation: { outputs: { voicemonkey: { enabled: true } } } };
            process.env.VOICEMONKEY_TOKEN = 'env-token';
        ...
        it('should not overwrite existing secrets', () => {
            const config = { automation: { outputs: { voicemonkey: { params: { token: 'existing' } } } } };
            process.env.VOICEMONKEY_TOKEN = 'env-token';
            ...
            expect(result.changed).toBe(false);
            expect(result.config.automation.outputs.voicemonkey.params.token).toBe('existing');
