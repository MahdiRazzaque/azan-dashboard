...
describe('migrationService Env Migration', () => {
    ...
    it('should migrate VoiceMonkey secrets from env', () => {
        ...
        const { config: result, changed, migratedKeys } = migrationService.migrateEnvSecrets(config);

        expect(changed).toBe(true);
        expect(result.automation.outputs.voicemonkey.params.token).toBe('envToken');
        expect(result.automation.outputs.voicemonkey.params.device).toBe('envDevice');
    ...
    it('should NOT overwrite existing secrets in config', () => {
        ...
        expect(result.automation.outputs.voicemonkey.params.token).toBe('existingToken');
