...
describe("AutomationService Comprehensive", () => {
  ...
  it("should cover all remaining branches", async () => {
    ...
    const mockStrategy = {
      ...
      constructor: {
        getMetadata: () => ({ id: "voicemonkey", timeoutMs: -5000 }),
      },
    };
  ...
  describe("getAudioSource", () => {
    ...
    it("should return null for file type with empty path", () => {
      const result = service.getAudioSource(
        { type: "file", path: "" },
        "fajr",
        "adhan",
      );
  ...
  describe("triggerEvent Error Paths", () => {
    it("should handle TTS generation failure", async () => {
      audioAssetService.ensureTTSFile.mockResolvedValue({
        success: false,
      ...
      expect(sseService.broadcast).toHaveBeenCalledWith(
        expect.objectContaining({
          payload: expect.objectContaining({
            message: expect.stringContaining("TTS Service Offline"),
          }),
        }),
      );
    });
    ...
    it("should handle missing custom file", async () => {
      configService.get.mockReturnValue({
        automation: {
          triggers: {
            fajr: {
              adhan: { enabled: true, type: "file", path: "missing.mp3" },
            },
          },
        },
      });
      ...
      expect(sseService.broadcast).toHaveBeenCalledWith(
        expect.objectContaining({
          payload: expect.objectContaining({
            message: expect.stringContaining("Custom file missing"),
          }),
        }),
      );
    });
    ...
    it("should reject custom file paths outside the audio directory", async () => {
      configService.get.mockReturnValue({
        automation: {
          triggers: {
            fajr: {
              adhan: { enabled: true, type: "file", path: "../outside.mp3" },
            },
          },
        },
      });
    ...
    it("should handle successful custom file access", async () => {
      configService.get.mockReturnValue({
        automation: {
          triggers: {
            fajr: {
              adhan: { enabled: true, type: "file", path: "exists.mp3" },
            },
          },
        },
      });
