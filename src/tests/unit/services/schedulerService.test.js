...
describe("SchedulerService", () => {
  ...
  it("should schedule and execute prayer triggered events", async () => {
    await service.initScheduler();
    const jobCall = schedule.scheduleJob.mock.calls.find(
      (c) => typeof c[0] === "object",
    );
  ...
  it("should respect global switches", async () => {
    ...
    await service.initScheduler();
    const automationJob = schedule.scheduleJob.mock.calls.find(
      (c) => typeof c[0] === "object",
    );
  ...
  it("should respect individual global switches", async () => {
    ...
    const automationJobs = schedule.scheduleJob.mock.calls.filter(
      (c) => typeof c[0] === "object" && !(c[0] instanceof String),
    );
  ...
  it("should hot reload by cancelling old jobs", async () => {
    await service.initScheduler();
    const initialJobs = schedule.scheduleJob.mock.results
      .map((r) => r.value)
      .filter((j) => j !== null);
  ...
  describe("Maintenance Jobs", () => {
    it("should run daily health check at 00:00", async () => {
      await service.initScheduler();
      const call = schedule.scheduleJob.mock.calls.find(
        (c) => c[0] === jobConstants.JOB_HEALTH_CHECK,
      );
      expect(call[1]).toBe("0 0 * * *");
      await call[2]();
      expect(healthCheck.runDailyMaintenance).toHaveBeenCalled();
    });
    ...
    it("should run stale check and force refresh if needed", async () => {
      ...
      await service.initScheduler();
      const call = schedule.scheduleJob.mock.calls.find(
        (c) => c[0] === jobConstants.JOB_STALE_CHECK,
      );
    ...
    it("should run year boundary check and fetch next year if needed", async () => {
      ...
      await service.initScheduler();
      const call = schedule.scheduleJob.mock.calls.find(
        (c) => c[0] === jobConstants.JOB_YEAR_BOUNDARY,
      );
      expect(call[1]).toBe("0 4 * * *");
      await call[2]();
      expect(prayerTimeService.getPrayerTimes).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({ year: 2100 }),
      );
    });
    ...
    it("should run audio asset maintenance daily", async () => {
      await service.initScheduler();
      const call = schedule.scheduleJob.mock.calls.find(
        (c) => c[0] === jobConstants.JOB_AUDIO_ASSETS,
      );
    ...
    it("should run midnight refresh", async () => {
      await service.initScheduler();
      const call = schedule.scheduleJob.mock.calls.find(
        (c) => c[0] === jobConstants.JOB_MIDNIGHT_REFRESH,
      );
      expect(call[1]).toBe("0 0 * * *");
      await call[2]();
    ...
    it("should handle maintenance job errors", async () => {
      ...
      healthCheck.runDailyMaintenance.mockRejectedValue(
      ...
      // Trigger Stale Check
      const staleCall = schedule.scheduleJob.mock.calls.find(
        (c) => c[0] === jobConstants.JOB_STALE_CHECK,
      );
      await staleCall[2]();

      // Trigger Health Check
      const healthCall = schedule.scheduleJob.mock.calls.find(
        (c) => c[0] === jobConstants.JOB_HEALTH_CHECK,
      );
      await healthCall[2]();
    ...
    it("should handle missing cache meta in stale check", async () => {
      ...
      await service.initScheduler();
      const staleJobCall = schedule.scheduleJob.mock.calls.find(
        (c) => c[0] === jobConstants.JOB_STALE_CHECK,
      );
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining("No cache meta found"),
      );
    });

    it("should execute midnight refresh", async () => {
      await service.initScheduler();
      const midnightCall = schedule.scheduleJob.mock.calls.find(
        (c) => c[0] === jobConstants.JOB_MIDNIGHT_REFRESH,
      );
      ...
      expect(configService.reload).toHaveBeenCalled();
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining("Midnight Refresh"),
      );
    });
    ...
    it("should handle year boundary error", async () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("Year Boundary Check Failed"),
        expect.anything(),
      );
    });
  });
  ...
  describe("getJobs formatting", () => {
    ...
    it("should handle different date types in nextInvocation", async () => {
      ...
      expect(list.automation[0].nextInvocation).toBe(
    ...
    it("should handle all date fallback types in formatJob", async () => {
      ...
      expect(service.getJobs().maintenance[0].nextInvocation).toBe(
        DateTime.fromJSDate(date).toISO(),
      );
    ...
    it("should skip year boundary check if far from end of year", async () => {
      ...
      await service.initScheduler();
      const boundaryJobCall = schedule.scheduleJob.mock.calls.find(
        (c) => c[0] === jobConstants.JOB_YEAR_BOUNDARY,
      );
  ...
  describe("initScheduler edge cases", () => {
    it("should handle failure in getPrayerTimes", async () => {
      ...
      await service.initScheduler();
      expect(console.error).toHaveBeenCalledWith(
        expect.stringContaining("Initialisation failed"),
        expect.anything(),
      );
    });
    ...
    it("should skip prayer if no time found", async () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(
        expect.stringContaining("No time found for fajr, skipping."),
      );
    });
    ...
    it("should skip events if disabled", async () => {
      ...
      const preAdhanJob = list.automation.find(
        (j) => j.name === "fajr - preAdhan",
      );
    ...
    it("should handle missing iqamah object in prayer data", async () => {
      ...
      await service.initScheduler();
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining("Initialisation complete"),
      );
    });
    ...
    it("should handle iqamah from source data", async () => {
      ...
      await service.initScheduler();
      const iqamahJob = schedule.scheduleJob.mock.calls.find(
        (c) =>
          c[0] instanceof Date &&
          c[0].toISOString() === "2099-01-01T05:30:00.000Z",
      );
    ...
    it("should handle iqamah override from config", async () => {
      ...
      const iqamahJob = schedule.scheduleJob.mock.calls.find(
        (c) =>
          c[0] instanceof Date &&
          c[0].toISOString() === "2099-01-01T05:15:00.000Z",
      );
      expect(iqamahJob).toBeDefined();
    });
    ...
    it("should handle sunrise without iqamah", async () => {
      ...
      configWithSunrise.automation.triggers.sunrise = {
        adhan: { enabled: true },
      };
      configService.get.mockReturnValue(configWithSunrise);

      await service.initScheduler();
      const job = schedule.scheduleJob.mock.calls.find(
        (c) =>
          c[0] instanceof Date &&
          c[0].toISOString() === "2099-01-01T06:00:00.000Z",
      );
    ...
    it("should adjust scheduled time based on maximum lead time (staggered launch)", async () => {
      ...
      const jobCall = schedule.scheduleJob.mock.calls.find(
        (c) =>
          c[0] instanceof Date &&
          c[0].toISOString() === expectedTime.toISOString(),
      );

      expect(jobCall).toBeDefined();
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining("Lead: 2000ms"),
      );
    });
  });
  ...
  describe("runJob", () => {
    it("should execute existing maintenance job and return success", async () => {
      healthCheck.runDailyMaintenance.mockResolvedValue({ success: true }); // Ensure success
      await service.initScheduler();
