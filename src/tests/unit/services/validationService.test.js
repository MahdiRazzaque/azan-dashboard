...
describe('ValidationService Comprehensive', () => {
    ...
    it('should format ZodError with path', async () => {
        ...
        await expect(validateConfigSource(mockConfig)).rejects.toThrow('field.subfield: invalid');
    });

    it('should handle non-ZodError in structural validation', async () => {
        mockAladhanClass.getConfigSchema().parse.mockImplementation(() => { throw new Error('Generic error'); });

        await expect(validateConfigSource(mockConfig)).rejects.toThrow('Validation Failed: Generic error');
    });
    ...
    it('should throw if coordinates are missing for source that requires them', async () => {
        ...
        mockAladhanClass.getMetadata.mockReturnValue({ id: 'aladhan', label: 'Aladhan', requiresCoordinates: true });
        mockAladhanClass.getConfigSchema().parse.mockReturnValue({});

        await expect(validateConfigSource(configNoCoords)).rejects.toThrow('Coordinates (Latitude/Longitude) are required');
    });
    ...
    it('should throw if coordinates are incomplete', async () => {
        ...
        await expect(validateConfigSource(configIncompleteCoords)).rejects.toThrow('Coordinates (Latitude/Longitude) are required');
    });
    ...
    it('should handle ProviderValidationError during connectivity test', async () => {
        ...
        const pve = new ProviderValidationError('API Error', { detail: 'some detail' }, true);
    ...
    it('should handle generic error during connectivity test', async () => {
        mockAladhanClass.getConfigSchema().parse.mockReturnValue({});
        const mockProvider = { getAnnualTimes: jest.fn().mockRejectedValue(new Error('Network fail')) };
        ProviderFactory.create.mockReturnValue(mockProvider);

        await expect(validateConfigSource(mockConfig)).rejects.toThrow('PRIMARY Source (Aladhan) Connection Failed: Network fail');
    });
    ...
    it('should handle e.errors in ZodError', async () => {
        ...
        await expect(validateConfigSource(mockConfig)).rejects.toThrow('Validation Failed: invalid');
    });
    ...
    it('should skip coordinates check if not required', async () => {
        mockAladhanClass.getMetadata.mockReturnValue({ id: 'aladhan', label: 'Aladhan', requiresCoordinates: false });
        mockAladhanClass.getConfigSchema().parse.mockReturnValue({});
        const mockProvider = { getAnnualTimes: jest.fn().mockResolvedValue({}) };
        ProviderFactory.create.mockReturnValue(mockProvider);
