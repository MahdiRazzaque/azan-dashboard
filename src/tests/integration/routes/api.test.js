...
jest.mock("fs", () => {
  ...
  return {
    ...
    existsSync: jest.fn((p) => {
      ...
      if (
        p &&
        (p.includes("audio/temp") ||
          p.includes("audio\\temp") ||
          p.includes("audio/custom") ||
          p.includes("audio\\custom"))
      ) {
...
jest.mock("@services/system/configurationWorkflowService", () => ({
  executeUpdate: jest
    .fn()
    .mockResolvedValue({ message: "Success", warnings: [], meta: {} }),
...
jest.mock("@services/core/schedulerService", () =>
  mockMockFactory.createMockSchedulerService(),
);
jest.mock("@services/core/prayerTimeService", () =>
  mockMockFactory.createMockPrayerTimeService(),
);
...
jest.mock("@services/core/automationService", () => ({
  ...
  getAudioSource: jest.requireActual("@services/core/automationService")
    .getAudioSource,
}));
const automationService = require("@services/core/automationService");

jest.mock("@services/system/sseService", () =>
  mockMockFactory.createMockSSEService(),
);
...
jest.mock("@services/system/audioAssetService", () => ({
  ...
  enrichMetadata: jest
    .fn()
    .mockResolvedValue({ duration: 10, protected: false }),
}));
...
jest.mock("@providers", () => {
  const mockAladhanClass = {
    ...
    getConfigSchema: jest.fn().mockReturnValue({
      parse: jest.fn().mockImplementation((data) => {
        if (
          data.type === "aladhan" &&
          (data.method === undefined || data.madhab === undefined)
        ) {
  ...
  return {
    ...
    ProviderConnectionError:
      jest.requireActual("@providers").ProviderConnectionError,
    ProviderValidationError:
      jest.requireActual("@providers").ProviderValidationError,
  };
});
...
jest.mock("@services/system/diagnosticsService", () =>
  mockMockFactory.createMockDiagnosticsService(),
);
...
jest.mock("@services/system/voiceService", () => ({
  getVoices: jest.fn(() => [
    { Name: "Test", ShortName: "T", Gender: "Male", Locale: "en-US" },
  ]),
  init: jest.fn(),
}));
...
describe("API Routes Integration", () => {
  ...
  describe("System Routes", () => {
    ...
    it("POST /api/system/preview-tts - should generate preview", async () => {
      ...
      if (res.status !== 200) {
        throw new Error(
          `Integration test failed with status ${res.status}: ${JSON.stringify(res.body)}`,
        );
      }
  ...
  describe("Settings Routes", () => {
    ...
    it("POST /api/settings/update - should fail on validation error", async () => {
      workflowService.executeUpdate.mockRejectedValueOnce(
        new Error("Validation Failed: API Down"),
      );
  ...
  describe("Prayers Route", () => {
    it("GET /api/prayers - should return prayers", async () => {
      ...
      const res = await request(app).get("/api/prayers").expect(200);
      ...
      prayerTimeService.getPrayersWithNext.mockResolvedValueOnce({
        ...
        prayers: {
          fajr: {
            ...
            iqamah: "2024-01-01T05:15:00Z",
          },
          isha: {
            start: "2024-01-01T19:00:00Z",
            iqamah: "2024-01-01T19:15:00Z",
          },
        },
        nextPrayer: {
          name: "fajr",
  ...
  describe("Prayer Routes Extra Coverage", () => {
    ...
    it("GET /api/prayers - should handle errors", async () => {
      prayerTimeService.getPrayersWithNext.mockRejectedValueOnce(
  ...
  describe("Settings Update Edge Cases", () => {
    ...
    it("POST /settings/refresh-cache - should fail if audio sync fails", async () => {
      const audioAssetService = require("@services/system/audioAssetService");
      audioAssetService.syncAudioAssets.mockRejectedValueOnce(
        new Error("Sync Fail"),
      );
  ...
  describe("MyMasjid Coverage", () => {
    it("POST /api/settings/update - should successfully update MyMasjid source", async () => {
      const newConfig = {
        ...
        sources: {
          primary: {
            type: "mymasjid",
            masjidId: "94f1c71b-7f8a-4b9a-9e1d-3b5f6a7b8c9d",
          },
        },
      };
      ...
      expect(workflowService.executeUpdate).toHaveBeenCalledWith(
        expect.objectContaining({
          sources: expect.objectContaining({
            primary: expect.objectContaining({ type: "mymasjid" }),
          }),
        }),
      );
    });
    ...
    it("POST /api/settings/update - should handle failures from workflow", async () => {
      workflowService.executeUpdate.mockRejectedValueOnce(
        new Error("Validation Failed: Missing ID"),
      );
      const newConfig = {
        sources: { primary: { type: "mymasjid", masjidId: "" } },
      };
