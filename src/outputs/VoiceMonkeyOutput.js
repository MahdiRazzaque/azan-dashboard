...
class VoiceMonkeyOutput extends BaseOutput {
  ...
  async _executeFromFile(payload, metadata, signal) {
    ...
    const audioUrl = this._resolvePublicUrl(
      payload.source.url,
      payload,
      prefix,
    );
  ...
  async _sendToApi(audioUrl, payload, prefix, signal) {
    const config = ConfigService.get();
    const token =
      (payload.params && payload.params.token) ||
      config.automation?.outputs?.voicemonkey?.params?.token;
    const device =
      (payload.params && payload.params.device) ||
    ...
    try {
      await VoiceMonkeyOutput.queue.schedule(() =>
        axios.get("https://api-v2.voicemonkey.io/announcement", {
  ...
  async healthCheck(requestedParams) {
    ...
    const token =
      (requestedParams && requestedParams.token) ||
      config.automation?.outputs?.voicemonkey?.params?.token;
    const baseUrl = config.automation?.baseUrl;

    if (!baseUrl || !baseUrl.startsWith("https://")) {
      console.log(
        "[Output: VoiceMonkey] Health: Offline (HTTPS Base URL required)",
      );
    ...
    try {
      const response = await VoiceMonkeyOutput.queue.schedule(() =>
        axios.get("https://api-v2.voicemonkey.io/announcement", {
          params: {
            token: token,
      ...
      console.log(
        `[Output: VoiceMonkey] Health: Offline (${e.message || "Error"})`,
      );
  ...
  async verifyCredentials(credentials) {
    ...
    if (!credentials.token || !credentials.device) {
      console.log(
        "[Output: VoiceMonkey] Verification failed: Missing token or device",
      );
    ...
    try {
      ...
      console.log(
        `[Output: VoiceMonkey] Verification failed: ${error.message}`,
      );
  ...
  validateTrigger(trigger, context) {
    ...
    const file = audioFiles.find(
      (f) =>
        ...
        (trigger.type === "tts" &&
          f.name === `tts_${prayer}_${triggerType}.mp3`),
    );
    ...
    if (file) {
      ...
      if (isCompatible === false) {
        const issues = file.compatibility?.voicemonkey?.issues || [
          "Unknown issues",
        ];
        warnings.push(
          `${niceName}: Audio incompatible with Alexa (${issues.join(", ")})`,
        );
      }
    }
  ...
  static _getValidationIssues(metadata) {
    ...
    if (metadata.mimeType && !allowedMimeTypes.includes(metadata.mimeType)) {
      issues.push(
        `Unsupported format: ${metadata.format} / ${metadata.codec} (${metadata.mimeType}). Alexa requires MP3, AAC, OGG, OPUS or WAV.`,
      );
      ...
      const isAAC =
        format.includes("aac") ||
        format.includes("adts") ||
        codec.includes("aac");

      if (!isMP3 && !isWAV && !isOGG && !isAAC) {
        issues.push(
          `Unsupported format: ${metadata.format} / ${metadata.codec}. Alexa requires MP3, AAC, OGG, OPUS or WAV.`,
        );
      }
    }
    ...
    if (metadata.bitrate) {
      ...
      if (bitrateKbps > 1411.2) {
        issues.push(
          `Bitrate too high: ${bitrateKbps.toFixed(2)} kbps (Max 1411.2 kbps)`,
        );
      }
    }
    ...
    if (metadata.sampleRate) {
      ...
      if (sampleRateKhz > 48) {
        issues.push(
          `Sample rate too high: ${sampleRateKhz.toFixed(2)} kHz (Max 48 kHz)`,
        );
      }
    }
    ...
    if (metadata.duration && metadata.duration > 240) {
      issues.push(
        `Duration too long: ${metadata.duration.toFixed(2)}s (Max 240s)`,
      );
    }
...
VoiceMonkeyOutput.queue.on("failed", (error, jobInfo) => {
  console.warn(
    `[Queue:VoiceMonkey] Job ${jobInfo.options.id} failed: ${error.message}`,
  );
});
