...
class BaseOutput {
  ...
  async execute(payload, metadata, signal) {
    if (!payload || typeof payload !== "object") {
      throw new Error(
        "BaseOutput.execute() requires payload to be a non-null object",
      );
    }
    ...
    if (!supported.includes(normalized.type)) {
      throw new Error(
        `Source type "${normalized.type}" is not supported by ${meta.label}`,
      );
    }
    ...
    if (normalized.type === "file") {
      try {
        ...
        throw new Error(
          `Audio file not found: ${path.basename(normalized.filePath)}`,
        );
      }
      const isCompatible = await this._checkSidecarCompatibility(
        normalized.filePath,
      );
  ...
  async _checkSidecarCompatibility(filePath) {
    ...
    const metaPath = path.normalize(
      `${SRC_PUBLIC_ROOT}${path.sep}${relativePath}.json`,
    );
    ...
    try {
      ...
      if (isCompatible === false) {
        console.warn(
          `[Output: ${meta.label}] Skipped: Audio incompatible with ${meta.label}`,
        );
        return false;
      }
    } catch {
  ...
  getSecretRequirementKeys() {
    ...
    return metadata.params.filter((p) => p.sensitive).map((p) => p.key);
  }
