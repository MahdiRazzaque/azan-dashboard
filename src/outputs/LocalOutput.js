...
class LocalOutput extends BaseOutput {
  ...
  async _playAudio(audioSource, payload, metadata, signal) {
    ...
    const audioPlayer =
      (payload.params && payload.params.audioPlayer) || "mpg123";
  ...
  async healthCheck(requestedParams) {
    ...
    const audioPlayer =
      (requestedParams && requestedParams.audioPlayer) ||
    ...
    try {
      ...
      if (process.platform === "linux") {
        ...
        if (isWsl) {
          console.log(
            "[Output: Local] WSL Environment detected - Bypassing /dev/snd check",
          );
        } else {
          ...
          if (!hasSnd) {
            ...
            if (isDocker) {
              console.log(
                "[Output: Local] Health: Offline (Docker: No Audio HW)",
              );
