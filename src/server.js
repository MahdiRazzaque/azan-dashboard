  ...
  return new Promise((resolve) => {
    const server = app.listen(port, async () => {
      ...
      if (!process.env.JWT_SECRET) {
        console.log(
          "[Startup] JWT_SECRET missing. Generating a robust 64-byte key...",
        );
      ...
      if (!process.env.ENCRYPTION_SALT) {
        console.log(
          "[Startup] ENCRYPTION_SALT missing. Generating a robust 32-byte salt...",
        );
      ...
      configService.setOutputStrategyResolver((id) =>
        require("@outputs").getStrategy(id),
      );
      configService.setOutputSecretKeysResolver(() =>
        require("@outputs").getSecretRequirementKeys(),
      );
      ...
      Object.entries(config.sources).forEach(([key, source]) => {
        ...
        switch (source.type) {
          ...
          case "aladhan":
            ...
            console.log(
              `  • ${key} (aladhan) → latitude: ${lat}, longitude: ${long}`,
            );
            break;
          default:
      ...
      try {
        ...
        console.error(
          "[Startup] Failed to synchronise audio assets:",
          e.message,
        );
      }
