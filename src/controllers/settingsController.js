...
const settingsController = {
  ...
  updateSettings: async (req, res) => {
    try {
      ...
      res
        .status(status)
        .json({ error: "Update Failed", message: error.message });
    }
  },
  ...
  resetSettings: async (req, res) => {
    ...
    try {
      ...
      console.log(
        "[SettingsController] local.json deleted. Reverting to default.",
      );
    } catch {
  ...
  refreshCache: async (req, res) => {
    ...
    if (!primaryHealth.healthy && !backupHealth.healthy) {
      return res.status(503).json({
        error:
          "System is relying on cache. Cannot reload cache until at least one Prayer API is online.",
      });
    }
    ...
    try {
      ...
      console.error(
        "[SettingsController] Audio asset synchronisation failed:",
        e.message,
      );
      return res.status(400).json({
        error: "Sync Failed",
  ...
  uploadFile: async (req, res) => {
    ...
    // nosemgrep: path-join-resolve-traversal -- req.file.filename is generated by multer (UUID-based), not raw user input
    ...
    try {
      ...
      if (existingFiles.length >= 500) {
        ...
        return res.status(400).json({
          error: "Limit Reached",
          message:
            "Maximum of 500 custom audio files allowed. Please delete some files before uploading more.",
        });
      }
      ...
      try {
        ...
        if (
          !basicMetadata.mimeType ||
          !basicMetadata.mimeType.startsWith("audio/")
        ) {
      ...
      const enrichedMetadata = await audioAssetService.enrichMetadata(
        targetPath,
        basicMetadata,
      );
      ...
      console.error(
        "[SettingsController] Upload processing failed:",
        error.message,
      );
      // Cleanup on error
      try {
  ...
  deleteFile: async (req, res) => {
    ...
    const isAudioContained =
      audioRelative === "" ||
      (!audioRelative.startsWith("..") && !path.isAbsolute(audioRelative));
    const isMetaContained =
      metaRelative === "" ||
      (!metaRelative.startsWith("..") && !path.isAbsolute(metaRelative));
    ...
    try {
      // nosemgrep: express-fs-filename -- filename is sanitised via sanitizeFilename() + containment check above; rule has no sanitizer support
      ...
      if (metadata.protected) {
        return res.status(403).json({
          error: "Forbidden: File is protected and cannot be deleted",
        });
      }
    } catch {
      /* ignore if meta missing or corrupt */
    }
    ...
    try {
      ...
      // nosemgrep: express-fs-filename -- filename is sanitised via sanitizeFilename() + containment check above; rule has no sanitizer support
      try {
        await fsAsync.access(audioPath);
        audioExists = true;
      } catch {
        /* ignore */
      }
      ...
      return res
        .status(500)
        .json({ error: "Internal Server Error: Failed to delete file" });
    }
