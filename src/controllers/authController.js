...
const authController = {
  ...
  setup: async (req, res) => {
    ...
    if (process.env.ADMIN_PASSWORD) {
      return res.status(403).json({
        error: "System already configured. Login to change settings.",
      });
    }
    ...
    try {
      ...
      const token = jwt.sign({ role: "admin", tokenVersion }, jwtSecret, {
        expiresIn: "24h",
      });
  ...
  login: async (req, res) => {
    ...
    if (isValid) {
      ...
      if (!secret) {
        return res.status(500).json({
          error: "System security not fully configured (Missing JWT Secret)",
        });
      }

      const tokenVersion = configService.get().security.tokenVersion;
      const token = jwt.sign({ role: "admin", tokenVersion }, secret, {
        expiresIn: "24h",
      });
