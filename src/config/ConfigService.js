...
class ConfigService {
  ...
  constructor() {
    ...
    this._localPath =
      process.env.LOCAL_CONFIG_PATH || path.join(__dirname, "local.json");
  ...
  setOutputStrategyResolver(resolver) {
    if (typeof resolver !== "function") {
      throw new TypeError(
        "[ConfigService] setOutputStrategyResolver() expects a function.",
      );
    }
  ...
  setOutputSecretKeysResolver(resolver) {
    if (typeof resolver !== "function") {
      throw new TypeError(
        "[ConfigService] setOutputSecretKeysResolver() expects a function.",
      );
    }
  ...
  async init() {
    ...
    if (!process.env.ENCRYPTION_SALT) {
      throw new CriticalConfigurationError(
        "ENCRYPTION_SALT environment variable is required for security.",
      );
    }
  ...
  async reload() {
    ...
    try {
      ...
      if (e.code !== "ENOENT") {
        console.warn(
          "Warning: Failed to load local config (using defaults):",
          e.message,
        );
      }
    }
    ...
    const {
      ...
      changed,
      migratedKeys,
    } = migrationService.migrateEnvSecrets(rawConfig);
    if (changed) {
      console.log(
        `[Config] Migrating ${migratedKeys.length} secrets from .env to local.json...`,
      );
  ...
  _validateConstraints(config) {
    ...
    if (!this._outputStrategyResolver) {
      throw new Error(
        "[ConfigService] Output strategy resolver not wired. Call setOutputStrategyResolver() before init().",
      );
    }

    for (const [id, outputConfig] of Object.entries(
      ...
      try {
        ...
        if (typeof outputConfig.leadTimeMs === "number") {
          ...
          if (outputConfig.leadTimeMs < min || outputConfig.leadTimeMs > max) {
            const clamped = Math.max(
              min,
              Math.min(outputConfig.leadTimeMs, max),
            );
            console.warn(
              `[Config] Warning: leadTimeMs for '${id}' (${outputConfig.leadTimeMs}ms) exceeds allowed range [${min}-${max}ms]. Clamped to ${clamped}ms.`,
            );
        ...
        if (id === "local" && outputConfig.params?.audioPlayer) {
          ...
          const playerParam = metadata.params.find(
            (p) => p.key === "audioPlayer",
          );
          ...
          if (!allowed.includes(outputConfig.params.audioPlayer)) {
            console.warn(
              `[Config] Warning: Invalid audioPlayer '${outputConfig.params.audioPlayer}' for 'local' output. Reverting to default 'mpg123'.`,
            );
  ...
  _applyEnvOverrides(config) {
    ...
    if (process.env.PYTHON_SERVICE_URL)
      config.automation.pythonServiceUrl = process.env.PYTHON_SERVICE_URL;

    // [Dynamic Output Secrets]
    if (!this._outputSecretKeysResolver) {
      throw new Error(
        "[ConfigService] Output secret keys resolver not wired. Call setOutputSecretKeysResolver() before init().",
      );
    }
  ...
  _stripSecrets(config) {
    ...
    if (config.automation) {
      ...
      if (process.env.PYTHON_SERVICE_URL)
        delete config.automation.pythonServiceUrl;
      ...
      secrets.forEach(({ strategyId, key }) => {
        ...
        if (process.env[envKey]) {
          ...
          if (
            config.automation.outputs?.[strategyId]?.params?.[key] !== undefined
          ) {
  ...
  async _processSensitiveFields(obj, action = "encrypt") {
    ...
    if (obj.automation?.outputs) {
      ...
      for (const [id, outputConfig] of Object.entries(obj.automation.outputs)) {
        try {
          ...
          const metadata = strategy.constructor.getMetadata();
          const sensitiveKeys =
            metadata.params?.filter((p) => p.sensitive).map((p) => p.key) || [];
              ...
              if (val && typeof val === "string") {
                if (action === "encrypt") {
                  ...
                  if (!val.includes(":") && !encryption.isMasked(val)) {
                    outputConfig.params[sKey] = await encryption.encrypt(
                      val,
                      key,
                    );
                  }
                  ...
                  if (val.includes(":")) {
                    try {
                      outputConfig.params[sKey] = await encryption.decrypt(
                        val,
                        key,
                      );
                    } catch {
    ...
    if (obj.sources) {
      let ProviderFactory;
      try {
        ProviderFactory = require("../providers").ProviderFactory;
      } catch {}
      ...
      if (ProviderFactory) {
        for (const role of ["primary", "backup"]) {
          ...
          if (source && source.type) {
            try {
              const providerClass = ProviderFactory.getProviderClass(
                source.type,
              );
