...
vi.mock("../../../../src/components/settings/ClientSettingsModal", () => ({
  default: ({ onClose }) => (
    <div data-testid="client-settings-modal">
...
describe("TopControls", () => {
  ...
  it("should toggle mute", () => {
    ...
    render(
      <MemoryRouter>
        <TopControls isMuted={false} toggleMute={toggleMute} blocked={false} />
      </MemoryRouter>,
    );
  ...
  it("should not show warning for unhealthy TTS service if TTS is not used", () => {
    ...
    render(
      <MemoryRouter>
        <TopControls isMuted={false} toggleMute={vi.fn()} blocked={false} />
      </MemoryRouter>,
    );
  ...
  it("should show system warning for TTS service if enabled and unhealthy", () => {
    ...
    expect(warnings[0].getAttribute("title")).toContain(
      "TTS Service: Service Down",
    );
  });
  ...
  it("should use fallback label for outputs without one", () => {
    useSettings.mockReturnValue({
      ...
      config: {
        automation: {
          ...
          triggers: {
            fajr: {
              azan: { enabled: true, type: "file", targets: ["voicemonkey"] },
            },
          },
        },
      },
    ...
    const warnings = screen.getAllByTitle(/System Warning/);
    expect(warnings[0].getAttribute("title")).toContain(
      "Voicemonkey is used by fajr azan but is OFFLINE",
    );
  });
    ...
    useSettings.mockReturnValue({
      ...
      config: {
        automation: {
          triggers: {
            fajr: {
              azan: { enabled: true, type: "file", targets: ["browser"] },
            },
          },
        },
      },
