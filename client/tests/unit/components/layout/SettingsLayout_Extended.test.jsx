...
vi.mock('../../../../src/components/common/ConfirmModal', () => ({ default: ({ isOpen, onConfirm, onClose, title }) => isOpen ? <div data-testid="confirm-modal">{title}<button onClick={onConfirm}>Confirm</button><button onClick={onClose}>Close</button></div> : null }));
vi.mock('../../../../src/components/common/SaveProcessModal', () => ({ default: ({ isOpen, onClose }) => isOpen ? <div data-testid="process-modal"><button onClick={onClose}>Close</button></div> : null }));

describe('SettingsLayout Extended Coverage', () => {
  const mockSettings = {
    config: { sources: { backup: { enabled: true } }, automation: { outputs: { alexa: { enabled: true } } } },
    draftConfig: { sources: { backup: { enabled: true } }, automation: { outputs: { alexa: { enabled: true } } } },
    systemHealth: { primarySource: { healthy: true }, backupSource: { healthy: true }, alexa: { healthy: true } },
  ...
  it('should handle validation failure in handleGlobalSave', async () => {
    ...
    mockSettings.validateBeforeSave.mockReturnValue({ success: false, error: 'Validation Failed' });
  ...
  it('should handle reset failure in handleGlobalReset', async () => {
    mockSettings.resetToDefaults.mockResolvedValue({ success: false, error: 'Reset Error' });
  ...
  it('should show unhealthy indicator for Automation tab when outputs are offline', () => {
    ...
    expect(automationLink.querySelector('.lucide-alert-triangle')).toBeDefined();
  });
  ...
  it('should handle sidebar toggle and overlay click', () => {
    ...
    // Open sidebar
    fireEvent.click(screen.getByRole('button', { name: '' }).parentElement.querySelector('.lucide-menu').closest('button'));
  ...
  it('should cover isDirty for Automation tab', () => {
    mockSettings.isSectionDirty.mockImplementation((path) => path === 'automation.outputs');
