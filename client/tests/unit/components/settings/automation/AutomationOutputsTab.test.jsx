...
vi.mock("../../../../../src/components/settings/OutputStrategyCard", () => ({
  default: ({ strategy, onChange }) => (
    <div data-testid="output-card">
      {strategy.label}
      <button onClick={() => onChange("enabled", true)}>Toggle</button>
    </div>
  ),
}));
...
describe("AutomationOutputsTab", () => {
  ...
  it("should call updateSetting when strategy card triggers onChange", () => {
    ...
    render(
      <AutomationOutputsTab {...defaultProps} updateSetting={updateSetting} />,
    );
    const toggleButton = screen.getAllByText("Toggle")[0];
    fireEvent.click(toggleButton);
    expect(updateSetting).toHaveBeenCalledWith(
      "automation.outputs.local.enabled",
      true,
    );
  });
});
