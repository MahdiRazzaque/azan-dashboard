...
describe('PrayerSourceStatusCard', () => {
  ...
  const mockProviders = [
    { id: 'aladhan', label: 'Aladhan', branding: { icon: 'Globe', accentColor: 'blue' } },
    { id: 'mymasjid', label: 'MyMasjid', branding: { icon: 'Database', accentColor: 'emerald' } }
  ];
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockImplementation((url) => {
        if (url === '/api/prayers') return Promise.resolve({ ok: true, json: () => Promise.resolve({ meta: { source: 'Aladhan' } }) });
  ...
  it('should render correctly and fetch active source', async () => {
    render(<PrayerSourceStatusCard config={{ sources: { primary: { type: 'aladhan' } } }} />);
  ...
  it('should handle connectivity test success', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Connection Success')).toBeDefined());
  ...
  it('should handle connectivity test failure', async () => {
    fetch.mockImplementation((url) => {
        if (url === '/api/prayers') return Promise.resolve({ ok: true, json: () => Promise.resolve({ meta: { source: 'Aladhan' } }) });
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ success: false, message: 'Rejected' }) });
    });
    render(<PrayerSourceStatusCard config={{ sources: { primary: { type: 'aladhan' } } }} />);
    await waitFor(() => screen.getByText('Aladhan'));
  ...
  it('should detect cache fallback', async () => {
    ...
    render(<PrayerSourceStatusCard config={{ sources: { primary: { type: 'aladhan' }, backup: { type: 'mymasjid', enabled: true } } }} />);
    await waitFor(() => expect(screen.getByText('Dashboard Cache')).toBeDefined());
  });
