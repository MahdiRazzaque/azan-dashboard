...
describe("ClientSettingsModal", () => {
  ...
  it("should handle switch components in Appearance", () => {
    ...
    const switches = screen
      .getAllByRole("button")
      .filter((b) => b.className.includes("inline-flex"));
  ...
  it("should handle the date navigation toggle in Appearance", () => {
    ...
    const row = screen
      .getByText("Date Navigation")
      .closest("div")?.parentElement;
    const toggle =
      row?.querySelector("button.inline-flex") ??
      container.querySelector("button.inline-flex");
  ...
  it("should handle individual audio exclusions and disabled reasons", () => {
    ...
    fireEvent.click(screen.getByText("Prayer Audio"));
    expect(
      screen.getAllByTitle("Automation is globally disabled").length,
    ).toBeGreaterThan(0);
    ...
    useSettings.mockReturnValue({
      config: {
        automation: {
          global: { enabled: true, adhanEnabled: false },
          triggers: { fajr: { adhan: { enabled: true } } },
        },
      },
    });
    ...
    expect(
      screen.getAllByTitle(/adhan events are disabled globally/i).length,
    ).toBeGreaterThan(0);
    ...
    rerender(<ClientSettingsModal onClose={onClose} />);
    expect(
      screen.getAllByTitle(/this automation is disabled system-wide/i).length,
    ).toBeGreaterThan(0);

    const muteButtons = screen
      .getAllByRole("button")
      .filter((b) => b.className.includes("w-10 h-10"));
