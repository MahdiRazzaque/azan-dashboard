...
vi.mock('../../../../../src/components/settings/developer/StatusCells', () => ({
  AutomationStatusCell: ({ status }) => <div data-testid="automation-status">{status}</div>,
...
describe('AutomationTTSTab', () => {
  ...
  const defaultProps = {
    config: { automation: { global: { enabled: true, preAdhanEnabled: true, adhanEnabled: true, preIqamahEnabled: true, iqamahEnabled: true } } },
    systemHealth: { tts: { healthy: true } },
    automationStatus: { 
        fajr: { preAdhan: { status: 'PASSED' }, adhan: { status: 'PASSED' }, preIqamah: { status: 'PASSED' }, iqamah: { status: 'PASSED' } },
        sunrise: { preAdhan: { status: 'PASSED' }, adhan: { status: 'PASSED' } }
    },
    ttsStatus: {
        fajr: { preAdhan: { status: 'GENERATED' }, adhan: { status: 'GENERATED' }, preIqamah: { status: 'GENERATED' }, iqamah: { status: 'GENERATED' } },
        sunrise: { preAdhan: { status: 'GENERATED' }, adhan: { status: 'GENERATED' } }
    },
    fetchDiagnostics
  };
  ...
  it('should handle refresh successfully', async () => {
    ...
    await waitFor(() => expect(screen.getAllByText('Refreshed').length).toBeGreaterThan(0));
  });
  ...
  it('should show sub-systems disabled opacity', () => {
    const props = {
        ...defaultProps,
        config: { automation: { global: { enabled: true, preAdhanEnabled: false } } }
    };
    render(<AutomationTTSTab {...props} />);
  ...
  it('should show loading state when status is null', () => {
    render(<AutomationTTSTab {...defaultProps} automationStatus={null} ttsStatus={null} />);
