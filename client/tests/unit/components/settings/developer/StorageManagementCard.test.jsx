...
describe('StorageManagementCard', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    useSettings.mockReturnValue({ draftConfig: null, updateSetting, saveSettings });
  ...
  it('should render storage stats after loading', async () => {
    render(<StorageManagementCard config={config} />);
    await waitFor(() => expect(screen.getByText(/50.0 MB \/ 100 MB/)).toBeDefined());
  ...
  it('should handle successful save', async () => {
    ...
    const saveButton = screen.getAllByRole('button').find(b => b.querySelector('.lucide-save'));

    fireEvent.change(input, { target: { value: '2.0' } });
    fireEvent.click(saveButton);

    expect(updateSetting).toHaveBeenCalledWith('data.storageLimit', 2);
    await waitFor(() => expect(screen.getByText('Limit updated')).toBeDefined());
  });
  ...
  it('should show amber color for high usage', async () => {
    fetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ ...mockStorage, usedBytes: 80 * 1024 * 1024 })
    });
    render(<StorageManagementCard config={config} />);
  ...
  it('should show red color for critical usage', async () => {
    fetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ ...mockStorage, usedBytes: 95 * 1024 * 1024 })
    });
    render(<StorageManagementCard config={config} />);
