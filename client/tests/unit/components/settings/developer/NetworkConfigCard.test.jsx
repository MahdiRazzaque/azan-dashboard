...
describe("NetworkConfigCard", () => {
  ...
  it("should handle successful save", async () => {
    ...
    await waitFor(() =>
      expect(updateEnvSetting).toHaveBeenCalledWith(
        "BASE_URL",
        "https://new-url.com",
      ),
    );
  ...
  it("should handle save failure", async () => {
    updateEnvSetting.mockResolvedValue({
      success: false,
      error: "Invalid URL according to server",
    });
    render(<NetworkConfigCard />);
    const input = screen.getByPlaceholderText("https://your-domain.com");
    const saveButton = screen.getByText("Save");
    ...
    await waitFor(() =>
      expect(screen.getByText("Invalid URL according to server")).toBeDefined(),
    );
  });
