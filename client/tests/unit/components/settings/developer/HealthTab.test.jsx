...
describe("HealthTab", () => {
  ...
  it("should show disabled status for disabled backup source", async () => {
    ...
    await act(async () => {
      render(
        <HealthTab
          config={configWithDisabledBackup}
          systemHealth={mockSystemHealth}
          refreshHealth={refreshHealth}
          refresh={refresh}
        />,
  ...
  it("should handle toggle monitoring for outputs", async () => {
    fetch.mockImplementation((url) => {
      ...
      if (url === "/api/system/outputs/registry") {
        return Promise.resolve({
          json: () => Promise.resolve([{ id: "alexa", label: "Alexa Output" }]),
        });
      }
      return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
    });
  ...
  it("should show online status when monitored but not yet checked", async () => {
    ...
    await act(async () => {
      render(
        <HealthTab
          config={configWithApiChecked}
          systemHealth={healthWithNoData}
          refreshHealth={refreshHealth}
          refresh={refresh}
        />,
      );
  ...
  it("should display per-service lastChecked timestamps, not global", async () => {
    const perServiceHealth = {
      ...
      tts: {
        healthy: false,
        message: "Unreachable",
        lastChecked: "2026-02-04T10:00:00Z",
      },
      primarySource: {
        healthy: true,
        message: "Healthy",
        lastChecked: "2026-02-04T08:00:00Z",
      },
      backupSource: { healthy: true, message: "Healthy", lastChecked: null },
      alexa: {
        healthy: true,
        message: "Healthy",
        lastChecked: "2026-02-04T14:00:00Z",
      },
    };
  ...
  it("should disable refresh button and daily check toggle for disabled output strategies", async () => {
    ...
    await act(async () => {
      render(
        <HealthTab
          config={configWithDisabledAlexa}
          systemHealth={mockSystemHealth}
          refreshHealth={refreshHealth}
          refresh={refresh}
        />,
    ...
    const alexaRow = screen
      .getByText("Alexa Output")
      .closest('div[class*="flex items-center justify-between"]');

    // The refresh button within the Alexa row should be disabled
    const refreshButton = alexaRow.querySelector(
      'button[title="Force Refresh Health"]',
    );
