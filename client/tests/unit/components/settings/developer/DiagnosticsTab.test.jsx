...
vi.mock('../../../../../src/components/settings/PrayerSourceStatusCard', () => ({ default: () => <div data-testid="source-card">Source Card</div> }));
vi.mock('../../../../../src/components/settings/developer/StorageManagementCard', () => ({ default: () => <div data-testid="storage-card">Storage Card</div> }));
vi.mock('../../../../../src/components/settings/developer/NetworkConfigCard', () => ({ default: () => <div data-testid="network-card">Network Card</div> }));
...
describe('DiagnosticsTab', () => {
  ...
  const defaultProps = {
    config: {},
    systemHealth: { tts: { healthy: true }, ports: { api: '3000', tts: '8000' } },
    apiOnline: true,
    refreshing: null,
    handleManualRefresh,
    feedback: {},
    ...
    jobStatuses: {},
    jobs: [
      { name: 'Job 1', nextInvocation: '2026-01-30T15:00:00' }
    ]
  };
  ...
  it('should handle manual refresh of services', () => {
    render(<DiagnosticsTab {...defaultProps} />);
    const refreshButtons = screen.getAllByRole('button').filter(b => b.querySelector('.lucide-refresh-cw'));
  ...
  it('should handle system actions', () => {
    ...
    expect(callSystemAction).toHaveBeenCalledWith('tts', '/api/system/regenerate-tts');

    fireEvent.click(screen.getByText('Restart Scheduler'));
    expect(callSystemAction).toHaveBeenCalledWith('scheduler', '/api/system/restart-scheduler');
  });
  ...
  it('should disable action buttons when loading', () => {
    ...
    const ttsButton = screen.getByText('Regenerate TTS Assets').closest('button');
    expect(ttsButton.disabled).toBe(true);
    
    const restartButton = screen.getByText('Restart Scheduler').closest('button');
  ...
  it('should show refreshing state for services', () => {
    ...
    const apiSection = screen.getByText('API Server').closest('div').parentElement;
