...
vi.mock("../../../../src/components/settings/DynamicField", () => ({
  default: ({ param, onChange }) => (
    <div data-testid="dynamic-field">
      {param.label}
      <button onClick={() => onChange("new-val")}>Change</button>
    </div>
  ),
}));
...
describe("SourceConfigurator", () => {
  ...
  const mockProviders = [
    {
      id: "aladhan",
      label: "Aladhan",
      parameters: [{ key: "method", label: "Method", default: 2 }],
      branding: { accentColor: "blue" },
      capabilities: { providesIqamah: false },
    },
    {
      id: "mymasjid",
      label: "MyMasjid",
      parameters: [],
      branding: { accentColor: "emerald" },
      capabilities: { providesIqamah: true },
    },
  ];
  ...
  it("should render provider buttons", () => {
    render(
      <SourceConfigurator source={{ type: "aladhan" }} onChange={onChange} />,
    );
  ...
  it("should show offline warning for active provider", () => {
    useSettings.mockReturnValue({
      systemHealth: {
        primarySource: { healthy: false, message: "Offline Error" },
      },
    });
  ...
  describe("Iqamah capability badge (FEAT-001)", () => {
    it("should display Iqamah badge on providers with providesIqamah capability", () => {
      ...
      expect(
        mymasjidButton.querySelector('[data-testid="iqamah-badge"]'),
      ).not.toBeNull();
    });
    ...
    it("should not display Iqamah badge on providers without providesIqamah", () => {
      ...
      expect(
        aladhanButton.querySelector('[data-testid="iqamah-badge"]'),
      ).toBeNull();
    });
    ...
    it("should show tooltip text on Iqamah badge", () => {
      render(
        <SourceConfigurator
          source={{ type: "mymasjid" }}
          onChange={onChange}
        />,
      );
      const badge = screen.getByTestId("iqamah-badge");
      expect(badge.getAttribute("title")).toBe(
        "This source provides Iqamah times",
      );
    });
    ...
    it("should not show badge when capabilities are undefined", () => {
      ...
      render(
        <SourceConfigurator source={{ type: "custom" }} onChange={onChange} />,
      );
