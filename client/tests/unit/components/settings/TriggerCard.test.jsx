...
vi.mock('../../../../src/components/common/SearchableSelect', () => ({ default: ({ placeholder, onChange, options }) => (
...
describe('TriggerCard', () => {
  ...
  const defaultProps = {
    label: 'Test Trigger',
    trigger: { enabled: true, type: 'file', path: 'audio.mp3', targets: ['local'] },
    onChange,
    files: [{ name: 'audio.mp3', path: 'audio.mp3', type: 'audio/mpeg' }],
    eventType: 'adhan',
    strategies: [{ id: 'local', label: 'Local' }, { id: 'voicemonkey', label: 'Alexa', compatibilityKey: 'alexaCompatible' }]
  };

  const mockSettings = {
    systemHealth: { tts: { healthy: true }, local: { healthy: true } },
    config: { automation: { global: { enabled: true }, outputs: { local: { enabled: true } } } },
    voices: []
  };
  ...
  it('should handle master switch disable (Sub-system)', () => {
    useSettings.mockReturnValue({
        ...mockSettings,
        config: { automation: { global: { enabled: true, adhanEnabled: false } } }
    });
  ...
  it('should toggle enabled state', () => {
    ...
    expect(onChange).toHaveBeenCalledWith({ ...defaultProps.trigger, enabled: false });
  });

  it('should change type', () => {
    render(<TriggerCard {...defaultProps} />);
    fireEvent.click(screen.getByText('TTS'));
    expect(onChange).toHaveBeenCalledWith({ ...defaultProps.trigger, type: 'tts' });
  });
  ...
  it('should show offset input for pre-events', () => {
    ...
    fireEvent.change(screen.getByLabelText('Minutes Before'), { target: { value: '20' } });
    expect(onChange).toHaveBeenCalledWith({ ...defaultProps.trigger, offsetMinutes: 20 });
  });

  it('should show dirty indicator', () => {
    const { container } = render(<TriggerCard {...defaultProps} isDirty={true} />);
  ...
  it('should show service warnings for disabled output', () => {
    useSettings.mockReturnValue({
        ...mockSettings,
        config: { automation: { global: { enabled: true }, outputs: { local: { enabled: false } } } }
    });
    render(<TriggerCard {...defaultProps} />);
  ...
  it('should show service warnings for offline output', () => {
    ...
    render(<TriggerCard {...defaultProps} />);
    expect(screen.getByText('Local Output Offline: Bad Connection')).toBeDefined();
  });

  it('should show warning for incompatible file', () => {
    const props = {
        ...defaultProps,
        trigger: { ...defaultProps.trigger, type: 'file', path: 'bad.mp3', targets: ['voicemonkey'] },
        files: [{ name: 'bad.mp3', path: 'bad.mp3', alexaCompatible: false, alexaIssues: ['Low Bitrate'] }]
    };
  ...
  it('should toggle targets', () => {
    ...
    expect(onChange).toHaveBeenCalledWith({ ...defaultProps.trigger, targets: ['local', 'voicemonkey'] });
    
    const localLabel = screen.getByText('Local');
    fireEvent.click(localLabel);
    expect(onChange).toHaveBeenCalledWith({ ...defaultProps.trigger, targets: [] });
  });

  it('should render extra content', () => {
    render(<TriggerCard {...defaultProps} extraContent={<div data-testid="extra">Extra</div>} />);
  ...
  it('should handle TTS inputs', () => {
    ...
    expect(onChange).toHaveBeenCalledWith({ ...props.trigger, template: 'New Template' });
  });

  it('should handle URL input', () => {
    const props = { ...defaultProps, trigger: { enabled: true, type: 'url', url: 'http://' } };
    render(<TriggerCard {...props} />);
    const input = screen.getByPlaceholderText('https://...');
    fireEvent.change(input, { target: { value: 'https://test.mp3' } });
    expect(onChange).toHaveBeenCalledWith({ ...props.trigger, url: 'https://test.mp3' });
  });
