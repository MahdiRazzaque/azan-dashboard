...
vi.mock('../../../../src/components/settings/AudioConsentModal', () => ({ default: ({ isOpen, onConfirm }) => isOpen ? <div data-testid="consent-modal"><button onClick={() => onConfirm(true)}>Confirm</button></div> : null }));
...
describe('OutputStrategyCard', () => {
  ...
  const strategy = {
    ...
    params: [
        ...
        { key: 'quality', label: 'Quality', type: 'select', options: ['High', 'Low'] }
    ],
    defaultLeadTimeMs: 0
  };
  ...
  it('should render correctly', () => {
    render(<OutputStrategyCard strategy={strategy} config={{ enabled: true }} onChange={onChange} />);
  ...
  it('should handle lead time adjustment with snapping', () => {
    render(<OutputStrategyCard strategy={strategy} config={{ enabled: true, leadTimeMs: 0 }} onChange={onChange} />);
  ...
  it('should handle parameter changes', () => {
    render(<OutputStrategyCard strategy={strategy} config={{ params: { device: 'old-device' } }} onChange={onChange} />);
  ...
  it('should handle health check returning unhealthy with message', async () => {
    fetch.mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ local: { healthy: false, message: 'Device Error' } })
    });
    render(<OutputStrategyCard strategy={strategy} config={{}} onChange={onChange} />);
    fireEvent.click(screen.getByText('Check Health'));
    await waitFor(() => expect(screen.getByText('Offline')).toBeDefined());
  });
  ...
  it('should handle missing systemHealth in sync', () => {
    render(<OutputStrategyCard strategy={strategy} config={{ enabled: true }} systemHealth={null} onChange={onChange} />);
    expect(screen.getByText('Local Speaker')).toBeDefined();
  });

  it('should show correct lead time labels and styles', () => {
    const { rerender } = render(<OutputStrategyCard strategy={strategy} config={{ leadTimeMs: 5000 }} onChange={onChange} />);
    expect(screen.getByText('+5.0s')).toBeDefined();
    expect(screen.getByText('+5.0s').className).toContain('text-emerald-400');

    rerender(<OutputStrategyCard strategy={strategy} config={{ leadTimeMs: -5000 }} onChange={onChange} />);
    expect(screen.getByText('-5.0s')).toBeDefined();
    expect(screen.getByText('-5.0s').className).toContain('text-amber-400');

    rerender(<OutputStrategyCard strategy={strategy} config={{ leadTimeMs: 0 }} onChange={onChange} />);
    expect(screen.getByText('Synchronised')).toBeDefined();
  });

  it('should handle missing target in systemHealth in sync', () => {
    render(<OutputStrategyCard strategy={strategy} config={{ enabled: true }} systemHealth={{ other: {} }} onChange={onChange} />);
    expect(screen.getByText('Local Speaker')).toBeDefined();
  });
  ...
  it('should return null if hidden', () => {
    ...
    const { container } = render(<OutputStrategyCard strategy={hiddenStrategy} config={{}} onChange={onChange} />);
