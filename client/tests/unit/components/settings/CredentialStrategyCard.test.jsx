...
describe("CredentialStrategyCard", () => {
  ...
  it("should handle reset flow", async () => {
    ...
    await waitFor(() =>
      expect(onSave).toHaveBeenCalledWith({ token: "" }, true),
    );
  });
  ...
  it("should handle verification flow", async () => {
    ...
    await waitFor(() =>
      expect(onSave).toHaveBeenCalledWith({ token: "val" }, false),
    );
    expect(screen.getByText("Verified & Saved")).toBeDefined();
  });
  ...
  it("should handle verification trigger failure", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText("Trigger failed")).toBeDefined(),
    );
  });
  ...
  it("should return null if no sensitive params", () => {
    const noSensitive = {
      ...strategy,
      params: [{ key: "p", sensitive: false }],
    };
    const { container } = render(
      <CredentialStrategyCard strategy={noSensitive} onSave={onSave} />,
    );
