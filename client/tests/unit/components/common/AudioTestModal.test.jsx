...
describe('AudioTestModal', () => {
  ...
  it('should return null if not open or no file', () => {
    const { container, rerender } = render(<AudioTestModal {...defaultProps} isOpen={false} />);
  ...
  it('should fetch and render strategies', async () => {
    ...
    await waitFor(() => expect(screen.getByText('All Dashboards')).toBeDefined());
  ...
  it('should toggle consent', () => {
    ...
    render(<AudioTestModal {...defaultProps} setConsentGiven={setConsentGiven} />);
  ...
  it('should disable unhealthy or disabled targets', async () => {
    useSettings.mockReturnValue({
        systemHealth: { local: { healthy: true }, voicemonkey: { healthy: false } },
        config: { automation: { outputs: { local: { enabled: false }, voicemonkey: { enabled: true } } } }
    });
    render(<AudioTestModal {...defaultProps} consentGiven={true} />);
    ...
    expect(screen.getByText('This output is disabled in settings')).toBeDefined();
  ...
  it('should render healthy and enabled non-browser targets', async () => {
    ...
    expect(screen.getByText('Plays on the mosque local audio system')).toBeDefined();
  });
