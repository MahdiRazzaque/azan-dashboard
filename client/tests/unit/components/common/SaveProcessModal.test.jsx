...
describe("SaveProcessModal", () => {
  ...
  it("should render processing state", () => {
    ...
    expect(
      screen.getByText(/This process involves generating audio files/),
    ).toBeDefined();
  });
  ...
  it("should render default processing state when processStatus is null", () => {
    ...
    expect(
      screen.getByText(/Please wait while we update the system/),
    ).toBeDefined();
  });
  ...
  it("should render warning state", () => {
    const result = {
      success: true,
      warning: true,
      warningsList: ["Warning 1", "Warning 2"],
    };
    render(
      <MemoryRouter>
  ...
  it("should render error state without warningsList", () => {
    render(
      <MemoryRouter>
        <SaveProcessModal
          {...defaultProps}
          result={{ success: false, error: "Failed big time" }}
        />
      </MemoryRouter>,
    );
  ...
  it("should handle single warning string", () => {
    render(
      <MemoryRouter>
        <SaveProcessModal
          {...defaultProps}
          result={{ success: true, warning: "Single warning" }}
        />
      </MemoryRouter>,
    );
  ...
  it("should handle null warningsList", () => {
    render(
      <MemoryRouter>
        <SaveProcessModal
          {...defaultProps}
          result={{ success: true, warning: "Single", warningsList: null }}
        />
      </MemoryRouter>,
    );
