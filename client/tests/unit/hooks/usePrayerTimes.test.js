...
const createPrayerDay = (dateKey) => ({
  fajr: {
    start: `${dateKey}T05:00:00.000Z`,
    iqamah: `${dateKey}T05:15:00.000Z`,
  },
  sunrise: { start: `${dateKey}T06:30:00.000Z` },
  dhuhr: {
    start: `${dateKey}T12:00:00.000Z`,
    iqamah: `${dateKey}T12:15:00.000Z`,
  },
  asr: {
    start: `${dateKey}T15:30:00.000Z`,
    iqamah: `${dateKey}T15:45:00.000Z`,
  },
  maghrib: {
    start: `${dateKey}T18:00:00.000Z`,
    iqamah: `${dateKey}T18:10:00.000Z`,
  },
  isha: {
    start: `${dateKey}T19:30:00.000Z`,
    iqamah: `${dateKey}T19:45:00.000Z`,
  },
});
...
const createCalendar = (startDate, count) => {
  ...
  for (let offset = 0; offset < count; offset += 1) {
    const dateKey = DateTime.fromISO(startDate)
      .plus({ days: offset })
      .toISODate();
...
const createResponse = ({
  ...
  nextPrayer: {
    name: "dhuhr",
    time: `${referenceDate}T12:00:00.000Z`,
    isTomorrow: false,
  },
...
describe("usePrayerTimes", () => {
  ...
  it("hydrates the current and viewed prayer data from the initial calendar response", async () => {
    ...
    expect(result.current.viewedPrayers).toEqual(
      response.calendar["2026-01-30"],
    );
  ...
  it("navigates within the cached calendar without triggering another fetch", async () => {
    ...
    expect(result.current.viewedPrayers).toEqual(
      response.calendar["2026-01-31"],
    );
    const directionalCalls = fetch.mock.calls.filter(([url]) =>
      String(url).includes("cursorDate="),
    );
  ...
  it("fetches and merges a future chunk when navigating beyond the cached window", async () => {
    ...
    const directionalCalls = fetch.mock.calls.filter(([url]) =>
      String(url).includes("cursorDate=2026-01-31"),
    );
  ...
  it("ignores repeated reset-to-today requests while a reset transition is already running", async () => {
    ...
    fetch.mockImplementation(() =>
      Promise.resolve(okResponse(initialResponse)),
    );
