...
describe('useSSE', () => {
  ...
  it('should handle AUDIO_PLAY messages and call onAudioPlay', () => {
    ...
    expect(result.current.logs[0].message).toContain('Playing Audio: Fajr azan');
  });
  ...
  it('should handle PROCESS_UPDATE messages', () => {
    ...
    act(() => {
      mockEventSource.onmessage({
        data: JSON.stringify({ type: 'PROCESS_UPDATE', payload: processPayload })
      });
    });
  ...
  it('should handle parse errors', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('SSE Parse Error', expect.any(Error));
