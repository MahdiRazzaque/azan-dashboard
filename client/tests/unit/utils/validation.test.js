...
describe('validation.js', () => {
  describe('validateTrigger', () => {
    ...
    it('should validate TTS triggers', async () => {
      expect(await validateTrigger({ enabled: true, type: 'tts', template: '' })).toBe('TTS template is required');
      expect(await validateTrigger({ enabled: true, type: 'tts', template: '  ' })).toBe('TTS template is required');
      expect(await validateTrigger({ enabled: true, type: 'tts', template: 'Hello' })).toBeNull();
    });

    it('should validate file triggers', async () => {
      expect(await validateTrigger({ enabled: true, type: 'file', path: '' })).toBe('A file must be selected');
      expect(await validateTrigger({ enabled: true, type: 'file', path: 'audio.mp3' })).toBeNull();
    });

    it('should validate URL triggers', async () => {
      expect(await validateTrigger({ enabled: true, type: 'url', url: '' })).toBe('URL is required');
      expect(await validateTrigger({ enabled: true, type: 'url', url: 'http://example.com/audio.wav' })).toBe('URL must point to an .mp3 file');
      
      // Mock successful fetch
      fetch.mockResolvedValueOnce({
        json: () => Promise.resolve({ valid: true })
      });
      expect(await validateTrigger({ enabled: true, type: 'url', url: 'http://example.com/audio.mp3' })).toBeNull();

      // Mock invalid URL from server
      fetch.mockResolvedValueOnce({
        json: () => Promise.resolve({ valid: false, error: 'Not found' })
      });
      expect(await validateTrigger({ enabled: true, type: 'url', url: 'http://example.com/audio.mp3' })).toBe('URL unreachable: Not found');

      // Mock invalid URL from server without error message
      fetch.mockResolvedValueOnce({
        json: () => Promise.resolve({ valid: false })
      });
      expect(await validateTrigger({ enabled: true, type: 'url', url: 'http://example.com/audio.mp3' })).toBe('URL unreachable: Unknown error');

      // Mock fetch error
      fetch.mockRejectedValueOnce(new Error('Network error'));
      expect(await validateTrigger({ enabled: true, type: 'url', url: 'http://example.com/audio.mp3' })).toBe('Validation check failed (Network error)');
    });
  ...
  describe('validateSourceSettings', () => {
    ...
    it('should validate required fields', () => {
      ...
      expect(validateSourceSettings(source, providerMetadata)).toBe('API Key is required');
    ...
    it('should validate pattern constraints', () => {
      ...
      expect(validateSourceSettings(source, providerMetadata)).toBe('Invalid format for Station ID');
