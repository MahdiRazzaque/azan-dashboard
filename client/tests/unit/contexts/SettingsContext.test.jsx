...
const baseConfig = {
  ...
  sources: {
    primary: { type: "aladhan", enabled: true },
    ...
    triggers: {
      fajr: {
        adhan: {
          ...
          offsetMinutes: 15,
          type: "tts",
          targets: ["local", "voicemonkey", "browser"],
        },
      },
    },
  },
...
describe("SettingsContext Ultimate Coverage", () => {
  const defaultFetchMock = async (url) => {
    ...
    if (url.includes("/api/system/health/refresh"))
      return mockResponse({ tts: { healthy: true } });
    if (url.includes("/api/system/health"))
      return mockResponse({ tts: { healthy: true }, local: { healthy: true } });
    if (url.includes("/api/system/voices")) return mockResponse([{ id: "v1" }]);
    if (url.includes("/api/system/providers"))
      return mockResponse([{ id: "aladhan" }]);
    if (url.includes("/api/settings/update"))
      return mockResponse({ success: true });
    if (url.includes("/api/settings/env"))
      return mockResponse({ success: true });
  ...
  it("should handle all initialization and state", async () => {
    ...
    await waitFor(() =>
      expect(fetch).toHaveBeenCalledWith(
        expect.stringContaining("/api/settings/public"),
        expect.any(Object),
      ),
    );
  });
  ...
  it("should cover all validation branches", async () => {
    ...
    render(
      <SettingsProvider>
        <TestComponent callback={(s) => (context = s)} />
      </SettingsProvider>,
    );
    expect(await screen.findByTestId("done")).toBeDefined();
    ...
    // Validation failures
    await act(async () => {
      context.updateSetting("location.coordinates.lat", "100");
    });
    await context.saveSettings();
    await act(async () => {
      context.updateSetting("location.coordinates.lat", "50");
      context.updateSetting("location.coordinates.long", "200");
    });
    await context.saveSettings();
    await act(async () => {
      context.updateSetting("location.coordinates.long", "0");
      context.updateSetting("location.timezone", "Invalid");
    });
    await context.saveSettings();

    await act(async () => {
      context.updateSetting("location.timezone", "Europe/London");
    });
  ...
  it("should cover utility methods and errors", async () => {
    ...
    act(() => {
      context.bulkUpdateOffsets("adhan", 10);
    });
    act(() => {
  ...
  describe("bulkUpdateIqamahOffsets (FEAT-005)", () => {
    ...
    const IqamahTestComponent = ({ callback }) => {
      ...
      return (
        <div data-testid="done">
          <span data-testid="fajr-offset">
            {s.draftConfig?.prayers?.fajr?.iqamahOffset}
          </span>
          <span data-testid="sunrise-offset">
            {s.draftConfig?.prayers?.sunrise?.iqamahOffset}
          </span>
          <span data-testid="dhuhr-offset">
            {s.draftConfig?.prayers?.dhuhr?.iqamahOffset}
          </span>
          <span data-testid="asr-offset">
            {s.draftConfig?.prayers?.asr?.iqamahOffset}
          </span>
          <span data-testid="maghrib-offset">
            {s.draftConfig?.prayers?.maghrib?.iqamahOffset}
          </span>
          <span data-testid="isha-offset">
            {s.draftConfig?.prayers?.isha?.iqamahOffset}
          </span>
          <span data-testid="isha-override">
            {String(s.draftConfig?.prayers?.isha?.iqamahOverride)}
          </span>
        </div>
      );
    };
    ...
    it("should update iqamahOffset for all prayers except sunrise", async () => {
      ...
      render(
        <SettingsProvider>
          <IqamahTestComponent callback={(s) => (context = s)} />
        </SettingsProvider>,
      );
      expect(await screen.findByTestId("done")).toBeDefined();

      act(() => {
        context.bulkUpdateIqamahOffsets(25);
      });
    ...
    it("should preserve iqamahOverride boolean (Safe Mode)", async () => {
      ...
      act(() => {
        context.bulkUpdateIqamahOffsets(30);
      });
    ...
    it("should clamp values to 0-60 range", async () => {
      ...
      act(() => {
        context.bulkUpdateIqamahOffsets(100);
      });
      await waitFor(() => {
      ...
      act(() => {
        context.bulkUpdateIqamahOffsets(-5);
      });
      await waitFor(() => {
    ...
    it("should handle NaN input gracefully", async () => {
      ...
      act(() => {
        context.bulkUpdateIqamahOffsets("abc");
      });
      await waitFor(() => {
        expect(screen.getByTestId("fajr-offset").textContent).toBe("0");
      });
  ...
  describe("Health check fetch on mount (Issue #37)", () => {
    const HealthTestComponent = ({ callback }) => {
      ...
      return (
        <div data-testid="done">
          <span data-testid="health-tts">
            {String(s.systemHealth?.tts?.healthy)}
          </span>
          <span data-testid="health-local">
            {String(s.systemHealth?.local?.healthy)}
          </span>
          <span data-testid="health-voicemonkey">
            {String(s.systemHealth?.voicemonkey?.healthy ?? "undefined")}
          </span>
        </div>
      );
    };
    ...
    it("should fetch health state from backend on mount", async () => {
      ...
      const healthCalls = fetch.mock.calls.filter(
        ([url]) =>
    ...
    it("should populate systemHealth from API response, not optimistic defaults", async () => {
      ...
      render(
        <SettingsProvider>
          <HealthTestComponent callback={(s) => (context = s)} />
        </SettingsProvider>,
      );
      expect(await screen.findByTestId("done")).toBeDefined();
      ...
      await waitFor(() => {
        ...
        expect(context.systemHealth.tts.message).toBe(
    ...
    it("should parse health data from 503 responses (degraded system)", async () => {
      const healthData = {
        ...
        tts: {
          ...
          lastChecked: "2026-03-10T17:26:25.620Z",
        },
        voicemonkey: {
          healthy: false,
          message: "Token Missing",
          lastChecked: "2026-03-10T17:22:57.506Z",
        },
      };
      fetch.mockImplementation(async (url) => {
        // Backend returns 503 when critical services are unhealthy
        if (url.includes("/api/system/health"))
          return mockResponse(healthData, false, 503);
        if (url.includes("/api/settings")) return mockResponse(baseConfig);
        if (url.includes("/api/system/voices")) return mockResponse([]);
        if (url.includes("/api/system/providers")) return mockResponse([]);
        return mockResponse({});
