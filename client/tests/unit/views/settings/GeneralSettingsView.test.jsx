...
vi.mock("../../../../src/components/settings/SourceConfigurator", () => ({
  default: ({ source, onChange }) => (
    <div data-testid={`source-${source?.type || "none"}`}>
      {source?.type}
      <button onClick={() => onChange({ type: "mymasjid" })}>
        Switch to MyMasjid
      </button>
      <button onClick={() => onChange({ type: "aladhan" })}>
        Switch to Aladhan
      </button>
    </div>
  ),
...
describe("GeneralSettingsView", () => {
  ...
  it("should handle primary source change", () => {
    ...
    expect(updateSetting).toHaveBeenCalledWith("sources.primary", {
      type: "mymasjid",
    });
  });
  ...
  it("should handle primary source change with mutual exclusion", () => {
    ...
    expect(updateSetting).toHaveBeenCalledWith(
      "sources.backup",
      expect.objectContaining({ type: "aladhan" }),
    );
  });

  it("should handle primary source change when no alternative provider available", () => {
    useProviders.mockReturnValue({
      providers: [{ id: "aladhan", label: "Aladhan" }],
      loading: false,
    });
    useSettings.mockReturnValue({
      draftConfig: {
        sources: { primary: { type: "aladhan" }, backup: { type: "aladhan" } },
        location: {},
      },
      updateSetting,
      loading: false,
  ...
  it("should handle toggle backup when no alternative provider available", () => {
    ...
    expect(updateSetting).not.toHaveBeenCalledWith(
      "sources.backup",
      expect.anything(),
    );
  });
  ...
  it("should enable backup source", () => {
    ...
    expect(updateSetting).toHaveBeenCalledWith(
      "sources.backup",
      expect.objectContaining({ type: "mymasjid", enabled: true, p1: "d1" }),
    );
  });
