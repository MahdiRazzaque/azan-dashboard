...
vi.mock('../../../../src/components/settings/developer/DiagnosticsTab', () => ({ default: ({ handleManualRefresh, callSystemAction, handleRunJob, feedback }) => (
    <div data-testid="diagnostics-tab">
        ...
        <button onClick={() => callSystemAction('config', '/api/config')}>Call Config Action</button>
        <button onClick={() => callSystemAction('other', '/api/other')}>Call Other Action</button>
        <button onClick={() => handleRunJob('job1')}>Run Job</button>
        {feedback && Object.entries(feedback).map(([k, v]) => <div key={k}>{v}</div>)}
    </div>
) }));
...
vi.mock('../../../../src/components/settings/developer/HealthTab', () => ({ default: () => <div data-testid="health-tab">Health Tab</div> }));
...
describe('DeveloperSettingsView Extended Coverage', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockImplementation(async (url) => {
        if (url === '/api/system/jobs') return { ok: true, json: () => Promise.resolve({ maintenance: [] }) };
        if (url === '/api/system/status/automation') return { ok: true, json: () => Promise.resolve({}) };
        if (url === '/api/system/status/tts') return { ok: true, json: () => Promise.resolve({}) };
  ...
  it('should handle fetchJobs with array data', async () => {
    fetch.mockImplementation(async (url) => {
        if (url === '/api/system/jobs') return { ok: true, json: () => Promise.resolve([{ name: 'Job 1' }]) };
        return { ok: true, json: () => Promise.resolve({}) };
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
  });
  ...
  it('should handle fetchJobs with empty data', async () => {
    fetch.mockImplementation(async (url) => {
        if (url === '/api/system/jobs') return { ok: true, json: () => Promise.resolve({}) };
        return { ok: true, json: () => Promise.resolve({}) };
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
  });
  ...
  it('should handle fetchDiagnostics failure', async () => {
    ...
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
    await waitFor(() => expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to fetch diagnostics'), expect.anything()));
    consoleSpy.mockRestore();
  });

  it('should handle callSystemAction with warnings', async () => {
    fetch.mockImplementation(async (url) => {
        if (url === '/api/config') return { ok: true, json: () => Promise.resolve({ message: 'Done', warnings: ['W1'] }) };
        return { ok: true, json: () => Promise.resolve({}) };
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
  ...
  it('should handle callSystemAction failure', async () => {
    fetch.mockImplementation(async (url) => {
        if (url === '/api/other') return { ok: false, json: () => Promise.resolve({ error: 'Action Failed' }) };
        return { ok: true, json: () => Promise.resolve({}) };
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
  ...
  it('should handle handleRunJob failure', async () => {
    fetch.mockImplementation(async (url) => {
        if (url === '/api/system/run-job') return { ok: false, json: () => Promise.resolve({ message: 'Job Failed' }) };
        return { ok: true, json: () => Promise.resolve({}) };
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
  ...
  it('should handle message close click', async () => {
    fetch.mockImplementation(async (url) => {
        if (url === '/api/other') return { ok: true, json: () => Promise.resolve({ message: 'Success' }) };
        return { ok: true, json: () => Promise.resolve({}) };
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
    fireEvent.click(screen.getByText('Call Other Action'));
