...
const mockTriggerCard = vi.fn(
  ({ label, onChange, trigger, extraContent, error, isDirty }) => (
    <div
      data-testid={`trigger-card-${label.replace(/\s+/g, "-").toLowerCase()}`}
    >
...
const mockIqamahTimingCard = vi.fn(() => (
  <div data-testid="iqamah-timing-card" />
));
...
describe("PrayerSettingsView", () => {
  ...
  const config = {
    ...
    automation: {
      triggers: {
        fajr: {
          preAdhan: { enabled: true, targets: [] },
          adhan: { enabled: true, targets: [] },
          preIqamah: { enabled: true, targets: [] },
          iqamah: { enabled: true, targets: [] },
        },
        sunrise: {
          preAdhan: { enabled: true, targets: [] },
          adhan: { enabled: true, targets: [] },
  ...
  beforeEach(() => {
    ...
    useSettings.mockReturnValue({
      ...
      getSectionHealth: getSectionHealth.mockReturnValue({
        healthy: true,
        issues: [],
      }),
  ...
  it("should pass isDirty to IqamahTimingCard when prayer settings change", async () => {
    useSettings.mockReturnValue({
      ...
      draftConfig: {
        ...
        prayers: {
          ...config.prayers,
          fajr: { ...config.prayers.fajr, iqamahOffset: 99 },
        },
      },
    });
  ...
  it("should handle fetch errors gracefully", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      "Failed to fetch output strategies",
      expect.any(Error),
    );
  ...
  it("should show health issues in tooltip", async () => {
    const getSectionHealth = vi
      .fn()
      .mockImplementation((path) =>
        path === "prayers.fajr"
          ? { healthy: false, issues: [{ type: "Connection Error" }] }
          : { healthy: true, issues: [] },
      );
    useSettings.mockReturnValue({
      ...useSettings(),
  ...
  it("should NOT render notification toasts (no local save/validation)", async () => {
    ...
    expect(
      screen.queryByText(
        "Some automations were invalid and have been disabled.",
      ),
    ).toBeNull();
  });
