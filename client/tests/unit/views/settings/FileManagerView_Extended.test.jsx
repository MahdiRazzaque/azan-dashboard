...
vi.mock("../../../../src/components/common/AudioTestModal", () => ({
  default: ({ isOpen, onTest, onClose, file }) =>
    isOpen ? (
...
vi.mock("../../../../src/components/common/ConfirmModal", () => ({
  default: ({ isOpen, onConfirm, onCancel, onClose, title }) =>
    isOpen ? (
...
describe("FileManagerView Extended Coverage", () => {
  ...
  it("should reject non-audio files", async () => {
    ...
    expect(
      screen.getByText("Invalid file type. Please upload an audio file."),
    ).toBeDefined();
  });
  ...
  it("should render grouped cache files and handle interactions", async () => {
    ...
    fetch.mockImplementation(async (url) => {
      if (url === "/api/system/audio-files")
        return { ok: true, json: () => Promise.resolve({ files: manyFiles }) };
      return { ok: true, json: () => Promise.resolve([]) };
    });
