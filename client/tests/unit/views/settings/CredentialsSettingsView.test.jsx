...
vi.mock('../../../../src/components/common/PasswordInput', () => ({ default: ({ value, onChange, placeholder }) => <input type="password" value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} /> }));
vi.mock('../../../../src/components/settings/CredentialStrategyCard', () => ({ default: ({ strategy, onSave }) => (
    <div data-testid="strategy-card">
        {strategy.label}
        <button onClick={() => onSave({ token: 'new-token' }, false)}>Save</button>
...
describe('CredentialsSettingsView', () => {
  ...
  const mockStrategies = [
    { id: 'voicemonkey', label: 'VoiceMonkey', params: [{ key: 'token', sensitive: true }] },
    { id: 'other', label: 'Other', params: [{ key: 'public', sensitive: false }] }
  ];
  ...
  it('should fetch and filter strategies', async () => {
    render(<CredentialsSettingsView />);
    await waitFor(() => expect(screen.getByText('Integration Secrets')).toBeDefined());
  ...
  it('should return early if newPassword is empty', async () => {
    ...
    fireEvent.click(screen.getByText('Update Password'));
    expect(fetch).not.toHaveBeenCalledWith('/api/auth/change-password', expect.any(Object));
  });

  it('should handle failed password change with generic error', async () => {
    fetch.mockImplementation((url) => {
        if (url.includes('registry')) return Promise.resolve({ ok: true, json: () => Promise.resolve(mockStrategies) });
        if (url.includes('change-password')) return Promise.resolve({ ok: false, json: () => Promise.resolve({}) }); // No message or error
  ...
  it('should handle network error in password change', async () => {
    fetch.mockImplementation((url) => {
        if (url.includes('registry')) return Promise.resolve({ ok: true, json: () => Promise.resolve(mockStrategies) });
        if (url.includes('change-password')) return Promise.reject(new Error('Network Fail'));
        return Promise.resolve({ ok: true });
    });
  ...
  it('should handle strategy reset', async () => {
    ...
    const updateCall = vi.mocked(fetch).mock.calls.find(call => call[0].includes('settings/update'));
  ...
  it('should handle strategy save failure (verified status update)', async () => {
    ...
    fetch.mockImplementation((url) => {
        if (url.includes('registry')) return Promise.resolve({ ok: true, json: () => Promise.resolve(mockStrategies) });
        if (url.includes('settings/update')) return Promise.resolve({ ok: false, json: () => Promise.resolve({ message: 'Update Fail' }) });
        return Promise.resolve({ ok: true });
    });
