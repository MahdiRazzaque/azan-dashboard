...
describe('FileManagerView Final Push Fix v2', () => {
  const mockFiles = [
    ...
    { name: 'protected.mp3', path: 'custom/prot.mp3', url: '/url/prot', type: 'custom', metadata: { protected: true } },
    { name: 'tts_fajr_adhan.mp3', path: 'cache/fajr.mp3', url: '/url/fajr', type: 'cache' },
    { name: 'other.mp3', path: 'cache/other.mp3', url: '/url/other', type: 'cache' }
  ];
  ...
  beforeEach(() => {
    ...
    mockAudio = { play: vi.fn().mockResolvedValue(), pause: vi.fn(), src: '', onended: null };
  ...
  it('should cover all branches and functions including errors', async () => {
      ...
      await act(async () => { fireEvent.click(playBtn); }); // toggle off
      
      mockAudio.play.mockRejectedValueOnce(new Error('P'));
      await act(async () => { fireEvent.click(playBtn); }); // toggle on and fail
