...
vi.mock('../../../../src/components/settings/developer/DiagnosticsTab', () => ({ 
    default: ({ handleManualRefresh, callSystemAction, handleRunJob, jobs, feedback }) => (
        <div data-testid="diagnostics-tab">
            ...
            <button onClick={() => callSystemAction('tts', '/api/tts')}>Call TTS</button>
            <button onClick={() => callSystemAction('config', '/api/config')}>Call Config</button>
            <button onClick={() => handleRunJob('Job1')}>Run Job</button>
            {jobs.length > 0 ? <div>{jobs[0].name}</div> : <div>No active maintenance jobs</div>}
            {feedback.api && <div>{feedback.api}</div>}
            {feedback.tts && <div>{feedback.tts}</div>}
        </div>
    ) 
}));
vi.mock('../../../../src/components/settings/developer/AutomationTTSTab', () => ({ default: () => <div data-testid="automation-tab">Automation Tab</div> }));
vi.mock('../../../../src/components/settings/developer/SystemLogsTab', () => ({ default: () => <div data-testid="logs-tab">Logs Tab</div> }));
...
describe('DeveloperSettingsView', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    useSettings.mockReturnValue({ config: mockConfig, draftConfig: mockConfig, refresh, refreshHealth, loading: false, systemHealth: {} });
    useOutletContext.mockReturnValue({ logs: [] });
    vi.stubGlobal('fetch', vi.fn().mockImplementation((url) => {
        if (url === '/api/system/jobs') return Promise.resolve({ ok: true, json: () => Promise.resolve({ maintenance: [{ name: 'Job1' }] }) });
        if (url === '/api/system/status/automation') return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
        if (url === '/api/system/status/tts') return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
  ...
  it('should handle run job failure with error field', async () => {
    fetch.mockImplementation((url) => {
        if (url === '/api/system/run-job') return Promise.resolve({ ok: false, json: () => Promise.resolve({ error: 'Job Error' }) });
  ...
  it('should handle diagnostics partial failure (autoRes fail)', async () => {
    ...
    fetch.mockImplementation((url) => {
        if (url.includes('/status/automation')) return Promise.resolve({ ok: false });
        if (url.includes('/status/tts')) return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ maintenance: [] }) });
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
  ...
  it('should handle diagnostics partial failure (ttsRes fail)', async () => {
    const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    fetch.mockImplementation((url) => {
        if (url.includes('/status/automation')) return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
  ...
  it('should handle system actions successfully', async () => {
    fetch.mockImplementation((url) => {
        if (url === '/api/tts') return Promise.resolve({ ok: true, json: () => Promise.resolve({ success: true, message: 'TTS Rebuilt' }) });
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ maintenance: [] }) });
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
  ...
  it('should handle special config action', async () => {
    fetch.mockImplementation((url) => {
        if (url === '/api/config') return Promise.resolve({ ok: true, json: () => Promise.resolve({ success: true, message: 'Config Saved' }) });
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ maintenance: [] }) });
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
  ...
  it('should handle run job action', async () => {
    fetch.mockImplementation((url) => {
        if (url === '/api/system/run-job') return Promise.resolve({ ok: true, json: () => Promise.resolve({ success: true }) });
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ maintenance: [] }) });
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
  ...
  it('should handle fetchJobs with legacy array format', async () => {
    fetch.mockImplementation((url) => {
        if (url === '/api/system/jobs') return Promise.resolve({ ok: true, json: () => Promise.resolve([{ name: 'LegacyJob' }]) });
  ...
  it('should handle fetchJobs with automation only', async () => {
    fetch.mockImplementation((url) => {
        if (url === '/api/system/jobs') return Promise.resolve({ ok: true, json: () => Promise.resolve({ automation: [] }) });
        return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
  ...
  it('should handle fetchJobs error', async () => {
    const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    fetch.mockImplementation((url) => {
        if (url === '/api/system/jobs') return Promise.reject(new Error('Job fail'));
        return Promise.resolve({ ok: true, json: () => Promise.resolve({}) });
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
    await waitFor(() => expect(consoleSpy).toHaveBeenCalled());
  ...
  it('should handle system action with warnings', async () => {
    fetch.mockImplementation((url) => {
        if (url === '/api/tts') return Promise.resolve({ 
            ok: true, 
            json: () => Promise.resolve({ success: true, message: 'Done', warnings: ['W1'] }) 
        });
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ maintenance: [] }) });
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
  ...
  it('should handle diagnostics fetch failure', async () => {
    const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    fetch.mockImplementation((url) => {
        if (url.includes('/api/system/status')) return Promise.resolve({ ok: false });
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ maintenance: [] }) });
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
    await waitFor(() => expect(consoleSpy).toHaveBeenCalled());
  ...
  it('should handle system action failure with error field', async () => {
    fetch.mockImplementation((url) => {
        if (url === '/api/tts') return Promise.resolve({ ok: false, json: () => Promise.resolve({ error: 'Specific Error' }) });
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ maintenance: [] }) });
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
    fireEvent.click(screen.getByText('Call TTS'));
  ...
  it('should handle system action failure with no details', async () => {
    fetch.mockImplementation((url) => {
        if (url === '/api/tts') return Promise.resolve({ ok: false, json: () => Promise.resolve({}) });
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ maintenance: [] }) });
    });
    render(<MemoryRouter><DeveloperSettingsView /></MemoryRouter>);
    fireEvent.click(screen.getByText('Call TTS'));
