...
describe('LoginView', () => {
  ...
  it('should handle successful login and navigate to /settings', async () => {
    ...
    await waitFor(() => expect(mockLogin).toHaveBeenCalledWith('correctpassword'));
    expect(mockNavigate).toHaveBeenCalledWith('/settings', { replace: true });
  });

  it('should handle successful login and navigate to previous location', async () => {
    useLocation.mockReturnValue({ state: { from: { pathname: '/developer' } } });
    mockLogin.mockResolvedValue({ success: true });
    render(
      <MemoryRouter>
        <LoginView />
    ...
    await waitFor(() => expect(mockNavigate).toHaveBeenCalledWith('/developer', { replace: true }));
  });
