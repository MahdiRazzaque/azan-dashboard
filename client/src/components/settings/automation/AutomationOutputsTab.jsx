 ...
 */
export default function AutomationOutputsTab({
  strategies,
  formData,
  systemHealth,
  updateSetting,
  ...
  return (
    <section className="bg-app-card p-6 rounded-lg border border-app-border shadow-md">
      ...
      <p className="text-sm text-app-dim mb-6">
        Configure audio output targets. Enable strategies to route audio to
      ...
      <div className="grid grid-cols-1 gap-6">
        {strategies.map((strategy) => (
          <OutputStrategyCard
            ...
            onChange={(field, val) =>
              updateSetting(`automation.outputs.${strategy.id}.${field}`, val)
            }
          />
        ))}
      </div>
