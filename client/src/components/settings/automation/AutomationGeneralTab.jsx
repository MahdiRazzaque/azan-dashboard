...
import { Power, Zap, CheckCircle } from "lucide-react";
...
/**
 * AutomationGeneralTab component handles master controls and batch adjustments.
 *
 * @param {object} props - The component props.
 * @param {object} props.config - The original configuration object.
 ...
 */
export default function AutomationGeneralTab({
  config,
  formData,
  onChange,
  bulkUpdateOffsets,
  ...
  const handleBulkUpdate = (type) => {
    ...
    setToast(
      `Successfully updated ${count} ${type.replace(/([A-Z])/g, " $1").toLowerCase()} triggers.`,
    );
  ...
  const handleIqamahBulkUpdate = () => {
    ...
    const primaryProvider = providers?.find(
      (p) => p.id === sources?.primary?.type,
    );
    const backupProvider = sources?.backup?.enabled
      ? providers?.find((p) => p.id === sources?.backup?.type)
      : null;
    const anyProvidesIqamah =
      primaryProvider?.capabilities?.providesIqamah ||
      backupProvider?.capabilities?.providesIqamah;

    const prayers = ["fajr", "dhuhr", "asr", "maghrib", "isha"];
    const anyOverrideOff =
      anyProvidesIqamah &&
      prayers.some((p) => !formData?.prayers?.[p]?.iqamahOverride);

    let message = `Successfully updated ${count} iqamah offset values.`;
    if (anyOverrideOff) {
      message +=
        " Note: Your active source provides Iqamah times. Prayers without override enabled will use source times instead.";
    }
    setToast(message);
    setTimeout(() => setToast(null), 5000);
  };
    ...
    <div className="space-y-8">
      ...
      <section className="bg-app-card p-6 rounded-lg border border-app-border shadow-md">
        <h2 className="text-xl font-semibold mb-4 text-emerald-400 flex items-center gap-2 border-b border-app-border pb-2">
          ...
          Master Controls
          {JSON.stringify(config?.automation?.global) !==
            JSON.stringify(formData?.automation?.global) && (
        ...
        <div className="space-y-2">
          <Toggle
            ...
            onChange={(v) => onChange("automation.global.enabled", v)}
          />

          <div className="pt-4 mt-2 border-t border-app-border space-y-4">
            <h3 className="text-xs font-semibold text-app-dim uppercase tracking-wider">
              Sub-Systems Override
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-2">
              <Toggle
                ...
                onChange={(v) =>
                  onChange("automation.global.preAdhanEnabled", v)
                }
              />
              <Toggle
                label="Adhan Events"
                checked={formData.automation?.global?.adhanEnabled ?? true}
                onChange={(v) => onChange("automation.global.adhanEnabled", v)}
              />
              <Toggle
                label="Pre-Iqamah Events"
                checked={formData.automation?.global?.preIqamahEnabled ?? true}
                onChange={(v) =>
                  onChange("automation.global.preIqamahEnabled", v)
                }
              />
              <Toggle
                label="Iqamah Events"
                checked={formData.automation?.global?.iqamahEnabled ?? true}
                onChange={(v) => onChange("automation.global.iqamahEnabled", v)}
              />
            </div>
          </div>
        </div>
      ...
      <section className="bg-app-card p-6 rounded-xl border border-app-border shadow-xl">
        ...
        <p className="text-sm text-app-dim mb-6">
          Apply offset times to all prayers in one click. System will skip
          Sunrise for Iqamah adjustments.
        </p>
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-4 bg-app-bg/20 p-4 rounded-lg border border-app-border">
            <div className="flex-1">
              ...
              <div className="text-xs text-app-dim">
                Minutes before Iqamah delivery
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2 bg-app-card border border-app-border rounded px-2 py-1">
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-4 bg-app-bg/20 p-4 rounded-lg border border-app-border">
            <div className="flex-1">
              ...
              <div className="text-xs text-app-dim">
                Minutes after Adhan for Iqamah
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2 bg-app-card border border-app-border rounded px-2 py-1">
