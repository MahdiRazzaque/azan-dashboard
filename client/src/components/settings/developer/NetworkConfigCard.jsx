      ...
      <div className="space-y-4 relative z-10">
        <div className="space-y-2">
          <label className="text-xs font-bold text-app-dim uppercase tracking-wider flex justify-between">
            ...
            {baseUrl &&
              (isValid ? (
                <span className="text-emerald-500 flex items-center gap-1 normal-case font-normal">
                  <CheckCircle className="w-3 h-3" /> Valid HTTPS
                </span>
              ) : (
                <span className="text-red-400 flex items-center gap-1 normal-case font-normal">
                  <XCircle className="w-3 h-3" /> HTTPS Required
                </span>
              ))}
          </label>
            ...
            <input
              ...
              className={cn(
                ...
                baseUrl && !isValid
                  ...
                  : "border-app-border focus:ring-1 focus:ring-blue-500/50",
              )}
            />
            <button
              onClick={handleSave}
              disabled={
                saving || !isValid || baseUrl === config?.automation?.baseUrl
              }
        ...
        <div className="p-3 bg-blue-900/10 rounded-lg border border-blue-800/20 flex items-start gap-3">
          ...
          <div className="text-[10px] text-app-dim leading-relaxed">
            Required for{" "}
            <span className="text-app-text font-medium">VoiceMonkey</span> and{" "}
            <span className="text-app-text font-medium">Remote Access</span>.
