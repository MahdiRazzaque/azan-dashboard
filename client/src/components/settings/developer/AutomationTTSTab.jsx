...
export default function AutomationTTSTab({
  ...
  const isPreAdhanDisabled =
    config?.automation?.global?.preAdhanEnabled === false;
  const isAdhanDisabled = config?.automation?.global?.adhanEnabled === false;
  const isPreIqamahDisabled =
    config?.automation?.global?.preIqamahEnabled === false;
  ...
  const refreshButton = (
    <button
      ...
      className={cn(
        ...
        refreshStatus === "idle" &&
          "bg-app-card border-app-border hover:bg-app-card-hover text-app-dim hover:text-app-text",
        refreshStatus === "loading" &&
          "bg-app-card border-app-border text-app-dim cursor-wait",
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div
        className={cn(
          ...
          isGlobalDisabled &&
            "opacity-60 grayscale bg-app-bg/10 pointer-events-none",
        )}
      >
        {isGlobalDisabled && (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-center">
            <thead className="text-xs text-app-dim uppercase bg-app-bg/50">
              <tr>
                ...
                <th
                  className={cn(
                    "px-3 py-2",
                    isPreAdhanDisabled && "opacity-30",
                  )}
                >
                  Pre-Adhan
                </th>
                <th
                  className={cn("px-3 py-2", isAdhanDisabled && "opacity-30")}
                >
                  Adhan
                </th>
                <th
                  className={cn(
                    "px-3 py-2",
                    isPreIqamahDisabled && "opacity-30",
                  )}
                >
                  Pre-Iqamah
                </th>
                <th
                  className={cn("px-3 py-2", isIqamahDisabled && "opacity-30")}
                >
                  Iqamah
                </th>
              </tr>
            ...
            <tbody className="divide-y divide-app-border/50">
              {!automationStatus ? (
                <tr>
                  <td colSpan="5" className="p-4 text-center text-app-dim">
                    Loading...
                  </td>
                </tr>
              ) : (
                Object.entries(automationStatus).map(([prayer, events]) => (
                  <tr key={prayer}>
                    <td className="px-3 py-3 text-left font-medium text-app-dim capitalize">
                      {prayer}
                    </td>
                    <td
                      className={cn(
                ...
                Object.entries(ttsStatus).map(([prayer, events]) => (
                  <tr key={prayer}>
                    ...
                    <td className="px-2 py-2">
                      <TTSStatusCell {...events.preAdhan} />
                    </td>
                    <td className="px-2 py-2">
                      <TTSStatusCell {...events.adhan} />
                    </td>
                    <td className="px-2 py-2">
                      {prayer === "sunrise" ? (
                        <EmptyCell />
