        ...
        {systemServices.map((service) => (
          <HealthRow
            ...
            status={
              service.id === "api"
                ? "online"
                : systemHealth[service.id]?.healthy
                  ? "online"
                  : "offline"
            }
        ...
        {outputs
          ...
          .map((output) => (
            <HealthRow
              ...
              label={output.label}
              status={
                config.automation?.outputs?.[output.id]?.enabled === false
                  ? "disabled"
                  : systemHealth[output.id]?.healthy
                    ? "online"
                    : "offline"
              }
...
function HealthRow({
  ...
  const isConfigIssue =
    message && configWarnings.some((w) => message.includes(w));
    ...
    <div className="flex items-center justify-between p-3 bg-app-card rounded border border-app-border">
      ...
      <div className="flex items-center gap-3">
        ...
        {canToggle ? (
          <div
            className={cn(
              "flex items-center gap-2 px-2 border-r border-app-border/50",
...
function StatusIndicator({ status }) {
  switch (status) {
    ...
    case "disabled":
      return <ShieldAlert className="w-5 h-5 text-amber-500 opacity-50" />;
    case "not-monitored":
    default:
      return <Circle className="w-5 h-5 text-app-dim" />;
  }
}
