 ...
 * @param root0.results
 * @param root0.loading
 * @param root0.systemHealth
 * @param root0.providers
 * @param root0.testConnection
 */
function PrayerSourceStatusRow({
  label,
  source,
  target,
  disabled,
        ...
        {result && (
          ...
          >
            ...
            <span className="truncate">
              {result.success ? "Connection Success" : result.message}
            </span>
          </div>
        )}
...
export default function PrayerSourceStatusCard({ config }) {
    ...
    try {
      ...
      setResults((prev) => ({
        ...
        [target]: {
          ...
          message: data.success ? data.message : data.message || data.error,
        },
      }));

      // Refresh health cache to update "Online" badges immediately
      await refreshHealth(
        target === "primary" ? "primarySource" : "backupSource",
      );
    } catch (err) {
      setResults((prev) => ({
        ...prev,
  ...
  const isActivePrimary =
    activeSource &&
    activeSource.toLowerCase().includes(primary?.type?.toLowerCase());
  ...
  const getStatusColor = () => {
    ...
    const sourceMatch = activeSource
      .toLowerCase()
      .includes(primary?.type?.toLowerCase());
  ...
  return (
    <div className="bg-app-card/40 border border-app-border rounded-xl p-6 lg:col-span-2">
      ...
      <p className="mt-6 text-xs text-app-dim flex items-center gap-1.5 bg-app-card/50 p-3 rounded border border-app-border/50 italic">
        ...
        These tests occur independently of the dashboard cache. A successful
        test means the source is reachable.
      </p>
    </div>
  );
}
