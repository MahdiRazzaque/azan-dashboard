...
export default function IqamahTimingCard({
    ...
    const backupProvider = backupSource?.enabled ? providers.find(p => p.id === backupSource.type) : null;

    const primaryProvidesIqamah = primaryProvider?.capabilities?.providesIqamah === true;
    const backupProvidesIqamah = backupProvider?.capabilities?.providesIqamah === true;
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <h4 className="text-sm font-semibold text-app-text flex items-center gap-2 uppercase tracking-tight">
                        Iqamah Timing
                        {isDirty && (
                            <span data-testid="dirty-dot" className="w-2 h-2 rounded-full bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.6)]" />
                        )}
                    </h4>
                </div>
                ...
                <div className="px-5 pb-5 space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div 
                        ...
                        className={cn(
                            ...
                            bannerConfig.type === 'blue' && "bg-blue-900/20 border-blue-800/50 text-blue-300",
                            bannerConfig.type === 'green' && "bg-emerald-900/20 border-emerald-800/50 text-emerald-300",
                            bannerConfig.type === 'amber' && "bg-amber-900/20 border-amber-800/50 text-amber-300"
                        )}
                    >
                    ...
                    {anyProvidesIqamah && (
                        <div className="flex items-center justify-between pb-3 border-b border-app-border">
                            ...
                            <button
                                ...
                                onClick={(e) => {
                                    e.stopPropagation();
                                    updatePrayerConfig('iqamahOverride', !currentPrayerSettings.iqamahOverride);
                                }}
                    ...
                    {!(
                        (primaryProvidesIqamah && backupProvidesIqamah && !currentPrayerSettings.iqamahOverride) ||
                        (primaryProvidesIqamah && !backupEnabled && !currentPrayerSettings.iqamahOverride)
                    ) && (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            {currentPrayerSettings.fixedTime === null ? (
                                <>
                                    <div>
                                        <label htmlFor="iqamahOffset" className="block text-[10px] text-app-dim font-bold uppercase tracking-wider mb-2">Offset (minutes)</label>
                                        <input 
                                            id="iqamahOffset"
                                            type="number" 
                                    ...
                                    <div>
                                        <label htmlFor="roundTo" className="block text-[10px] text-app-dim font-bold uppercase tracking-wider mb-2">Round to nearest</label>
                                        <input 
                                            id="roundTo"
                                            type="number" 
                                ...
                                <div className="md:col-span-2">
                                    <label htmlFor="fixedTime" className="block text-[10px] text-app-dim font-bold uppercase tracking-wider mb-2">Fixed Time (HH:MM)</label>
                                    <input 
                                        id="fixedTime"
                                        type="time" 
