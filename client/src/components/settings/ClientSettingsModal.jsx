  ...
  return (
    <button
      ...
      className={cn(
        ...
        enabled ? "bg-app-accent" : "bg-app-card-hover",
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-app-card/20 rounded-2xl border border-app-border">
      <div className="flex flex-col gap-1">
        ...
        <span className="text-xs text-app-dim">{description}</span>
      </div>
      {children}
    </div>
  );
...
function AppearanceTab({ preferences, updateAppearance }) {
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <SettingRow title="Show Seconds" description="Display seconds on the main clock">
        <ToggleSwitch enabled={preferences.appearance.showSeconds} onChange={() => updateAppearance('showSeconds', !preferences.appearance.showSeconds)} />
      </SettingRow>

      <SettingRow title="Date Navigation" description="Enable swipe and arrow navigation between timetable days">
        <ToggleSwitch enabled={preferences.appearance.enableDateNavigation} onChange={() => updateAppearance('enableDateNavigation', !preferences.appearance.enableDateNavigation)} />
      </SettingRow>
...
function PrayerAudioTab({ config, isAudioExcluded, toggleAudioExclusion, muteAll, unmuteAll }) {
  return (
...
function SystemTab({ preferences, updateAppearance, wakeLock, onRestartTour }) {
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      <SettingRow title="Dashboard Tour" description="Restart the guided dashboard tour">
        <button
      ...
      <SettingRow title="Target Dhuhr after Fajr" description="Skip Sunrise in the main dashboard countdown">
        <ToggleSwitch enabled={preferences.appearance.skipSunriseCountdown} onChange={() => updateAppearance('skipSunriseCountdown', !preferences.appearance.skipSunriseCountdown)} />
      </SettingRow>
      ...
      <div className={cn(
        ...
        <ToggleSwitch 
          enabled={preferences.appearance.wakeLockAutoStart} 
          onChange={() => updateAppearance('wakeLockAutoStart', !preferences.appearance.wakeLockAutoStart)} 
          disabled={!wakeLock.isSupported} 
        />
      </div>

      <SettingRow title="Auto-unmute on Load" description="Attempt to enable audio automatically when page loads">
        <ToggleSwitch enabled={preferences.appearance.autoUnmute} onChange={() => updateAppearance('autoUnmute', !preferences.appearance.autoUnmute)} />
      </SettingRow>
    </div>
  );
}
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
            {activeTab === 'appearance' && (
              <AppearanceTab preferences={preferences} updateAppearance={updateAppearance} />
            )}

            {activeTab === 'prayers' && (
              <PrayerAudioTab config={config} isAudioExcluded={isAudioExcluded} toggleAudioExclusion={toggleAudioExclusion} muteAll={muteAll} unmuteAll={unmuteAll} />
            )}

            {activeTab === 'system' && (
              <SystemTab preferences={preferences} updateAppearance={updateAppearance} wakeLock={wakeLock} onRestartTour={handleRestartDashboardTour} />
            )}
          </div>
        </div>
      </div>
