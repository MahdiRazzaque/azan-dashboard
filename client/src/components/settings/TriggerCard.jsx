...
function getMasterSwitchState(config, eventType) {
...
function collectTriggerIssues(trigger, systemHealth, config, strategies, files) {
    const issues = [];
...
function TriggerWarningBadge({ issues }) {
...
function UnsavedIndicator({ isDirty }) {
...
function AudioSourceSelector({ triggerType, systemHealth, onUpdate }) {
    return (
        <div>
            <div className="text-xs font-bold text-app-dim uppercase tracking-wider block mb-2">Audio Source</div>
                    ...
                    return (
                        <button
                            ...
                            className={cn(
                                ...
                                isOffline && triggerType !== type && "border-amber-900/30 text-amber-600/50"
                            )}
                        >
                            {type.toUpperCase()}
                            {isOffline && <AlertTriangle className={cn("w-3 h-3 absolute -top-1 -right-1 bg-app-card rounded-full", triggerType === type ? "text-app-text" : "text-amber-500")} />}
                        </button>
                    );
                })}
            </div>
...
function AudioSourceInput({ trigger, files, error, config, voices, onUpdate }) {
    return (
        <div className="bg-app-bg/20 p-3 rounded-lg border border-app-border">
            ...
            {trigger.type === 'tts' && (
                <div className="space-y-1 mt-3">
                     <div className="text-[10px] font-bold text-app-dim uppercase tracking-wider">Voice Selection</div>
                    <SearchableSelect 
                        value={trigger.voice || ""}
                        placeholder={`Default (${config.automation?.defaultVoice || 'ar-DZ-IsmaelNeural'})`}
                        onChange={val => onUpdate('voice', val)}
                        options={[
...
function TargetSelector({ trigger, strategies, systemHealth, config, onToggleTarget }) {
    return (
        <div>
            <div className="text-xs font-bold text-app-dim uppercase tracking-wider block mb-2">Targets</div>
            <div className="flex flex-wrap gap-3">
                 {strategies.filter(s => !s.hidden && s.id !== 'browser').map(strategy => {
                     ...
                     const Icon = strategy.id === 'local' ? Server : Zap;
                     ...
                     return (
                         <label key={strategy.id} className={cn(
                             ...
                             <input 
                                ...
                                onChange={() => onToggleTarget(strategy.id)} 
                              />
 ...
 */
export default function TriggerCard({ label, trigger, onChange, files, error, isDirty, eventType, extraContent, strategies = EMPTY_STRATEGIES }) {
    const { systemHealth, config, voices } = useSettings();
    
    const masterSwitchState = getMasterSwitchState(config, eventType);
    ...
    const triggerIssues = (trigger.enabled && !isDisabledByMaster) 
        ? collectTriggerIssues(trigger, systemHealth, config, strategies, files) 
        : [];
            ...
            isDisabledByMaster 
             ...
             <div className={cn("flex items-center justify-between", (!trigger.enabled || isDisabledByMaster) ? "p-3" : "mb-4")}>
                 <div className="flex items-center gap-2">
                    <h4 className={cn("font-semibold transition-colors", isDisabledByMaster ? "text-app-dim" : "text-app-text")}>{label}</h4>
                     ...
                     <TriggerWarningBadge issues={triggerIssues} />
                  </div>
                  <div className="flex items-center gap-4">
                     {eventType?.startsWith('pre') && (
                        <div className="flex items-center gap-2 animate-in fade-in slide-in-from-left-2 transition-all">
                            <span className="text-[10px] text-app-dim font-bold uppercase tracking-wider">
                                Minutes Before
                            </span>
                            <input
                                type="number"
             ...
             {trigger.enabled && !isDisabledByMaster && (
                 <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                     <AudioSourceSelector triggerType={trigger.type} systemHealth={systemHealth} onUpdate={update} />
                     <AudioSourceInput trigger={trigger} files={files} error={error} config={config} voices={voices} onUpdate={update} />
                     
                     {error && <p className="text-xs text-red-400 mt-1">{error}</p>}
                     
                     <TargetSelector trigger={trigger} strategies={strategies} systemHealth={systemHealth} config={config} onToggleTarget={toggleTarget} />
