  ...
  const masterSwitchState = (() => {
    ...
    if (eventType) {
      ...
      if (key && config.automation.global[key] === false) {
        return {
          disabled: true,
          reason: `${eventType.replace(/([A-Z])/g, " $1").trim()} Disabled`,
        };
      }
    }
    ...
    >
      {isDisabledByMaster && (
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-app-bg/20 backdrop-blur-[1px]">
          <div className="bg-app-card/90 px-4 py-2 rounded border border-app-border text-xs font-bold text-app-dim uppercase tracking-wider shadow-xl flex items-center gap-2">
            ...
            Disabled by{" "}
            {masterSwitchState.reason.includes("Global")
              ? "Global Switch"
              : "Sub-System"}
          </div>
        </div>
      ...
      <div
        className={cn(
          ...
          !trigger.enabled || isDisabledByMaster ? "p-3" : "mb-4",
        )}
      >
        <div className="flex items-center gap-2">
          <h4
            className={cn(
              ...
              isDisabledByMaster ? "text-app-dim" : "text-app-text",
            )}
          >
            {label}
          </h4>
          ...
          {(() => {
            ...
            if (trigger.type === "tts" && !systemHealth.tts?.healthy)
              issues.push("TTS Service Offline");
            ...
            (trigger.targets || []).forEach((targetId) => {
              ...
              if (outputConfig && !outputConfig.enabled) {
                ...
                const strategy = strategies.find((s) => s.id === targetId);
                issues.push(
                  `${strategy?.label || targetId} Output Offline: ${health.message || "Service unreachable"}`,
                );
              }
            });
            ...
            (trigger.targets || []).forEach((targetId) => {
              const strategy = strategies.find((s) => s.id === targetId);
              if (
                strategy?.supportedSourceTypes &&
                !strategy.supportedSourceTypes.includes(sourceType)
              ) {
                issues.push(
                  `${strategy.label} does not support ${sourceType.toUpperCase()} sources`,
                );
              }
            });
              ...
              (trigger.targets || []).forEach((targetId) => {
                ...
                if (strategy?.compatibilityKey) {
                  ...
                  if (file && file[strategy.compatibilityKey] === false) {
                    const issuesKey = strategy.compatibilityKey.replace(
                      "Compatible",
                      "Issues",
                    );
                    issues.push(
                      `${strategy.label} Incompatible: ${file[issuesKey]?.join(", ") || "Unsupported properties"}`,
                    );
                  }
                }
              });
      ...
      {trigger.enabled && !isDisabledByMaster && (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
          {/* Type Selector */}
          <div>
            <label className="text-xs font-bold text-app-dim uppercase tracking-wider block mb-2">
              Audio Source
            </label>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      trigger.type === type
                        ? isOffline
                          ? "bg-amber-600 border-amber-500 text-app-text"
                          : "bg-emerald-600 border-emerald-500 text-app-text"
                        : "bg-app-bg border-app-border text-app-dim hover:bg-app-card-hover",
                      isOffline &&
                        trigger.type !== type &&
                        "border-amber-900/30 text-amber-600/50",
                    )}
                  >
                    {type.toUpperCase()}
                    {isOffline && (
                      <AlertTriangle
                        className={cn(
                          "w-3 h-3 absolute -top-1 -right-1 bg-app-card rounded-full",
                          trigger.type === type
                            ? "text-app-text"
                            : "text-amber-500",
                        )}
          ...
          <div className="bg-app-bg/20 p-3 rounded-lg border border-app-border">
            {trigger.type === "file" && (
              <SearchableSelect
                ...
                options={(() => {
                  ...
                  if (
                    trigger.path &&
                    !files.some((f) => f.path === trigger.path)
                  ) {
              ...
              <div className="space-y-1">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-app-dim">
                    Variables:{" "}
                    <code className="text-app-dim">{"{prayerEnglish}"}</code>,{" "}
                    <code className="text-app-dim">{"{prayerArabic}"}</code>,{" "}
                    <code className="text-app-dim">{"{minutes}"}</code>
                  </span>
                  <span
                    className={cn(
                      "text-[10px] font-mono",
                      (trigger.template?.length || 0) >
                        TTS_TEMPLATE_MAX_LENGTH * 0.8
                        ? "text-amber-500"
                        : "text-app-dim",
                    )}
                  >
            ...
            {trigger.type === "tts" && (
              <div className="space-y-1 mt-3">
                <label className="text-[10px] font-bold text-app-dim uppercase tracking-wider">
                  Voice Selection
                </label>
                <SearchableSelect
                  value={trigger.voice || ""}
                  ...
                  options={[
                    {
                      value: "",
                      label: `System Default (${config.automation?.defaultVoice || "Fallback"})`,
                      sublabel: "Inherit global setting",
                    },
                    ...(voices || []).map((v) => ({
                      value: v.ShortName,
                      label:
              ...
              {strategies
                ...
                .map((strategy) => {
                  ...
                  if (isSelected) {
                    ...
                    colorClasses =
                      "bg-app-bg border-amber-900/30 text-amber-600/50";
                  }
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      {hasWarning && (
                        <AlertTriangle className="w-3 h-3 text-amber-500 absolute -top-1 -right-1 bg-app-card rounded-full" />
                      )}
