...
export default function PasswordInput({ value, onChange, placeholder = "Enter password", showStrength = false }) {
    ...
    const strength = passedCount <= 2 ? 'Weak' : passedCount < 5 ? 'Medium' : 'Strong';
    const color = passedCount <= 2 ? 'text-red-400' : passedCount < 5 ? 'text-amber-400' : 'text-emerald-400';
    const barColor = passedCount <= 2 ? 'bg-red-500' : passedCount < 5 ? 'bg-amber-500' : 'bg-emerald-500';
