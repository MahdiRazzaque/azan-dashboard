...
const TopControls = ({ isMuted, toggleMute, blocked }) => {
  ...
  const [prevAutoStart, setPrevAutoStart] = useState(
  ...
  const healthIssues = useMemo(() => {
    ...
    if (systemHealth.primarySource && !systemHealth.primarySource.healthy) {
      issues.push(
        `Primary Source: ${systemHealth.primarySource.message || "Offline"}`,
      );
    }

    // 2. TTS Service (if enabled)
    const ttsTriggersEnabled =
      config.automation?.triggers &&
      Object.values(config.automation.triggers).some((prayerTriggers) =>
        Object.values(prayerTriggers).some(
          (trigger) => trigger.enabled && trigger.type === "tts",
        ),
      );
        ...
        if (trigger.enabled) {
          (trigger.targets || []).forEach((targetId) => {
            ...
            if (outputCfg && !outputCfg.enabled) {
              usedOutputIssues.add(
                `${label} is used by ${prayer} ${type} but is DISABLED`,
              );
            } else if (health && !health.healthy) {
              usedOutputIssues.add(
                `${label} is used by ${prayer} ${type} but is OFFLINE`,
              );
            }
          });
        }
  ...
  const getWakeLockTitle = () => {
    ...
    if (wakeLock.error)
      return `Error: ${wakeLock.error.message}. Click to retry.`;
    if (wakeLock.isActive) return "Wake Lock Active (Screen will stay on)";
    if (preferences.appearance.wakeLockAutoStart && !wakeLock.isActive)
      return "Wake Lock paused for this session";
  ...
  return (
    <div className="absolute top-2 right-2 lg:top-6 lg:right-6 flex gap-1.5 lg:gap-3 z-50">
      <button
        ...
        className={`p-1.5 lg:p-3 rounded-full transition-all duration-300 shadow-lg backdrop-blur-md ${
          !wakeLock.isSupported
            ...
            : wakeLock.error
              ? "bg-app-danger/20 text-app-danger animate-pulse border border-app-danger/50"
              : wakeLock.isActive
      ...
      <button
        ...
        className={`p-1.5 lg:p-3 rounded-full transition-all duration-300 shadow-lg backdrop-blur-md ${
          blocked
            ...
            : isMuted
              ? "bg-app-card hover:bg-app-card/80 text-app-dim"
              : "bg-app-card hover:bg-app-card/80 text-app-accent"
        }`}
        title={
          blocked
      ...
      {showSettings && (
        <ClientSettingsModal onClose={() => setShowSettings(false)} />
      )}
    </div>
  );
};
