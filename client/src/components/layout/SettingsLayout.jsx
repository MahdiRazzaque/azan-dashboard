...
export default function SettingsLayout({ logs, processStatus }) {
  ...
  useEffect(() => {
    if (!config?.system?.tours || config.system.tours.adminSeen !== false)
      return;
  ...
  const handleGlobalReset = async () => {
    ...
    if (result.success) {
      setNotification({
        type: "success",
        message: "Factory defaults restored.",
      });
    } else {
      setNotification({
        type: "error",
  ...
  const handleStartAdminTour = useCallback(() => {
    ...
    requestAnimationFrame(() =>
      startTour("admin", adminTourSteps, handleAdminTourComplete),
    );
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: Clock,
      isDirty: () =>
        isSectionDirty("prayers") || isSectionDirty("automation.triggers"),
    },
    {
      ...
      isDirty: () => {
        ...
        return (
          ...
          isSectionDirty("automation.outputs")
        );
      },
    },
    {
  ...
  const getItemHealth = (item) => {
    ...
    if (item.label === "General") {
      ...
      if (
        config?.sources?.backup?.enabled &&
        systemHealth?.backupSource &&
        !systemHealth.backupSource.healthy
      ) {
  ...
  return (
    <div className="flex h-screen bg-app-bg text-app-text font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        {notification && (
          <div
            className={`absolute top-20 left-1/2 -translate-x-1/2 z-50 px-4 py-3 rounded-lg shadow-xl border backdrop-blur-md animate-in slide-in-from-top-2 fade-in max-w-lg w-full ${
