      ...
      >
        ...
        <div className="mb-6 relative">
          ...
          >
            ...
            {visualState === "success" && (
              <CheckCircle className="w-10 h-10 text-emerald-500 animate-in zoom-in spin-in-12" />
            )}
            {visualState === "warning" && (
              <AlertTriangle className="w-10 h-10 text-amber-500 animate-in zoom-in" />
            )}
            {visualState === "error" && (
              <XCircle className="w-10 h-10 text-red-500 animate-in zoom-in" />
            )}
          </div>
        ...
        >
          {visualState === "processing"
            ? processStatus || "Saving Configuration..."
            : visualState === "success"
              ? "Configuration Saved"
              : visualState === "warning"
                ? "Saved with Warnings"
        ...
        {visualState === "warning" && (
          <p className="text-app-dim text-sm mb-4 px-4">
            The configuration was saved, but some issues were detected (e.g.
            offline services or storage limits).
          </p>
        )}
        ...
        {visualState === "processing" && (
          <p className="text-app-dim text-sm animate-pulse">
            {processStatus
              ? "This process involves generating audio files and may take a moment."
          ...
          <div className="mt-8 flex flex-col w-full gap-2 animate-in fade-in fill-mode-forwards delay-100">
            ...
            <button
              ...
              className={cn(
                ...
                visualState === "success"
                  ? "bg-emerald-600 hover:bg-emerald-500 text-app-text shadow-emerald-900/20"
                  : visualState === "warning"
