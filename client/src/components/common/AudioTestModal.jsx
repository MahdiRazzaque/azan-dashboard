  ...
  const targets = strategies
    ...
    .map((strategy) => {
      ...
      if (!isEnabled) description = "This output is disabled in settings";
        ...
        description = `Offline: ${health?.message || "Service unreachable"}`;
      ...
      <div className="bg-app-card border border-app-border rounded-xl shadow-2xl w-full max-w-lg overflow-hidden relative z-10 animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center gap-3 p-3 bg-app-bg/50 rounded-lg border border-app-border/50">
            ...
            <div>
              <p className="text-xs text-app-dim uppercase tracking-wider font-bold">
                Filename
              </p>
          ...
          <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl flex gap-3">
            ...
            <div className="space-y-2">
              ...
              <p className="text-amber-200/70 text-xs leading-relaxed">
                Playback cannot be stopped once started. Ensure volume levels
                are safe before triggering audio on local audio systems or
                external devices.
              </p>
              <label className="flex items-center gap-2 cursor-pointer pt-1 group">
                <div className="relative">
                  ...
                  <div className="w-5 h-5 border-2 border-amber-500/50 rounded flex items-center justify-center peer-checked:bg-amber-500 peer-checked:border-amber-500 transition-all">
                    {consentGiven && (
                      <X className="w-3 h-3 text-app-card stroke-[4]" />
                    )}
                  </div>
                </div>
          ...
          <div className="space-y-3">
            <p className="text-xs text-app-dim uppercase tracking-wider font-bold px-1">
              ...
              {targets.map((target) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <span
                      className={`block font-bold ${!consentGiven || target.disabled ? "text-app-dim" : "text-app-text"}`}
                    >
                      {target.label}
                    </span>
