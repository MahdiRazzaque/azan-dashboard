...
export const SettingsProvider = ({ children }) => {
  ...
  const handleRateLimit = useCallback(() => {
    console.warn(
      "[SettingsContext] Rate limit hit. Pausing polling for 60 seconds.",
    );
  ...
  const saveSettings = useCallback(
    async (overrideConfig) => {
      ...
      try {
        const isEvent =
          overrideConfig &&
          (overrideConfig.nativeEvent ||
            typeof overrideConfig.preventDefault === "function");
        let configToSave =
          overrideConfig && !isEvent ? overrideConfig : draftConfig;
        ...
        if (configToSave.location) {
          ...
          if (isNaN(longNum) || longNum < -180 || longNum > 180) {
            return {
              success: false,
              error: "Validation Error: Longitude must be between -180 and 180",
            };
          }

          try {
            Intl.DateTimeFormat(undefined, {
              timeZone: configToSave.location.timezone,
            });
          } catch (e) {
            return {
              success: false,
        ...
        if (configToSave.sources) {
          ...
          for (const role of roles) {
            ...
            if (sourceError) {
              return {
                success: false,
                error: `Validation Error (${role.toUpperCase()} Source): ${sourceError}`,
              };
            }
          }
        }

        const prayers = configToSave.prayers
          ? Object.keys(configToSave.prayers)
          : [];
        let warningsList = [];
        let invalidCount = 0;
  ...
  const getSectionHealth = useCallback(
    (path) => {
      ...
      if (path.startsWith("prayers.")) {
        ...
        Object.entries(draftConfig.automation?.triggers || {}).forEach(
          ([prayer, triggers]) => {
  ...
  const updateEnvSetting = useCallback(
    async (key, value) => {
      ...
      try {
        ...
        if (res.ok) {
          const path =
            key === "BASE_URL" ? "automation.baseUrl" : `automation.${key}`;
