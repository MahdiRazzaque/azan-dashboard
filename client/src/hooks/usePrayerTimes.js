...
const sortDateKeys = (calendar) =>
  Object.keys(calendar || {}).sort((left, right) => left.localeCompare(right));
  ...
  return Object.fromEntries(
    Object.entries(calendar || {}).filter(([date]) => {
      const difference = Math.abs(
        Math.round(DateTime.fromISO(date).diff(reference, "days").days),
      );
...
export const usePrayerTimes = () => {
  ...
  const [navigationBoundaries, setNavigationBoundaries] = useState({
    past: false,
    future: false,
  });
  ...
  const handleMidnight = useCallback(
    (nextReferenceDate) => {
      const currentReferenceDate =
        referenceDateRef.current || initialReferenceDate || nextReferenceDate;
      const currentViewedDate = viewedDateRef.current;
      const nextViewedDate =
        !currentViewedDate || currentViewedDate === currentReferenceDate
          ? nextReferenceDate
          : currentViewedDate;
  ...
  const applyPayload = useCallback(
    (payload, options = {}) => {
      ...
      const currentReferenceDate =
        referenceDateRef.current || initialReferenceDate || null;
      const nextReference = getLaterDate(
        payloadReferenceDate,
      ...
      const isStalePayload = Boolean(
        payloadReferenceDate &&
        currentReferenceDate &&
        payloadReferenceDate < currentReferenceDate,
      );
      ...
      setNextPrayer((currentNextPrayer) => {
        if (isStalePayload) {
          return (
            getNextPrayerFromDay(mergedCalendar[nextReference]) ||
            currentNextPrayer
          );
        }
  ...
  const fetchPrayers = useCallback(
    async ({ isInitial = false, preserveViewedDate = false } = {}) => {
      if (isInitial) {
        setLoading(true);
      }
  ...
  const fetchCalendarChunk = useCallback(
    async (cursorDate, direction) => {
      ...
      try {
        const response = await fetch(
          buildPrayerUrl({ cursorDate, direction }),
          {
  ...
  const navigateDay = useCallback(
    async (delta) => {
      ...
      const targetDate = DateTime.fromISO(viewedDate)
        .plus({ days: delta })
        .toISODate();
  ...
  const resetViewedDate = useCallback(() => {
    if (
      !activeReferenceDate ||
      viewedDate === activeReferenceDate ||
    ...
    const nextDirection =
      viewedDate && viewedDate < activeReferenceDate ? "future" : "past";
  ...
  const transitionPrayers = transitionDate
    ? calendar[transitionDate] ||
      (transitionDate === activeReferenceDate ? prayers : null)
    : null;
  ...
  const canNavigateBackward = useMemo(() => {
    ...
    return (
      (minCachedDate && viewedDate > minCachedDate) ||
      !navigationBoundaries.past
    );
  ...
  const canNavigateForward = useMemo(() => {
    ...
    return (
      (maxCachedDate && viewedDate < maxCachedDate) ||
      !navigationBoundaries.future
    );
