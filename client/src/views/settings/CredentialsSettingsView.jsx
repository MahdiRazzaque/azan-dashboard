...
export default function CredentialsSettingsView() {
  ...
  const loadStrategies = () => {
    fetch("/api/system/outputs/registry")
      ...
      .then((data) =>
        setStrategies(data.filter((s) => s.params.some((p) => p.sensitive))),
      )
      .catch(console.error);
  };
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setPassMsg({
          type: "success",
          text: "Password updated. Logging out...",
        });
        setNewPassword("");
        setConfirmPassword("");
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-10">
      <div>
        ...
        <p className="text-app-dim mt-1">
          Manage admin access and integration secrets.
        </p>
      </div>
        ...
        <section className="space-y-6">
          ...
          <div className="bg-blue-500/10 border border-blue-500/20 p-4 rounded-lg text-sm text-blue-300 space-y-2">
            ...
            <p className="text-xs leading-relaxed opacity-90">
              Credentials are encrypted at rest and stored securely. For your
              protection, once a secret is saved, it will be masked (
              <span className="font-mono text-blue-200">********</span>) and
              cannot be viewed again. If you need to update a credential, simply
              enter the new value and verify it.
            </p>
            <p className="text-xs italic opacity-75">
              Note: You must verify credentials by hearing a test sound before
              they can be committed to the system.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            {strategies.map((strategy) => (
              <CredentialStrategyCard
                ...
                strategy={strategy}
                initialValues={
                  config?.automation?.outputs?.[strategy.id]?.params || {}
                }
                verified={config?.automation?.outputs?.[strategy.id]?.verified}
                onSave={(values, isReset) =>
                  handleStrategySave(strategy.id, values, isReset)
                }
              />
            ))}
          </div>
      ...
      <section className="bg-app-card p-6 rounded-lg border border-app-border shadow-md mt-8">
        ...
        <div className="max-w-md space-y-4">
          <div className="space-y-2">
            <label className="block text-sm font-medium text-app-dim">
              New Password
            </label>
            <PasswordInput
              value={newPassword}
          ...
          <div className="flex items-center gap-4 mt-6">
            ...
            {passMsg && (
              <span
                className={`text-sm font-medium ${passMsg.type === "success" ? "text-emerald-400 animate-pulse" : "text-red-400"}`}
              >
                {passMsg.text}
              </span>
            )}
