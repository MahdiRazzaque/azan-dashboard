...
export default function GeneralSettingsView() {
  const { draftConfig, updateSetting, loading, isSectionDirty, systemHealth } =
    useSettings();
  ...
  if (loading || !draftConfig)
    return (
      <div className="p-8 text-center text-app-dim">Loading settings...</div>
    );
  ...
  const backupMeta = backupEnabled
    ? providers.find((p) => p.id === backupSource.type)
    : null;
  const needsCoordinates =
    primaryMeta?.requiresCoordinates ||
    (backupEnabled && backupMeta?.requiresCoordinates);
  ...
  const handlePrimaryChange = (newSource) => {
    ...
    if (backupEnabled && backupSource.type === newSource.type) {
      const alternativeProvider = providers.find(
        (p) => p.id !== newSource.type,
      );
      if (alternativeProvider) {
        handleChange("sources.backup", {
          ...backupSource,
          type: alternativeProvider.id,
        });
      } else {
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-10">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-app-dim mt-1">
            Configure your prayer calculation source and location.
          </p>
        </div>
      </div>
      ...
      <section className="bg-app-card p-6 rounded-xl border border-app-border shadow-xl transition-all duration-300">
        <h2 className="text-xl font-semibold mb-6 text-emerald-400 border-b border-app-border pb-2 flex items-center gap-2">
          ...
          {!primaryHealthy && (
            <AlertTriangle
              ...
              title={systemHealth.primarySource.message || "Offline"}
            />
          )}
        ...
        <SourceConfigurator
          ...
          onLocationChange={(field, val) =>
            handleChange(`location.coordinates.${field}`, val)
          }
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-blue-400 flex items-center gap-2">
            ...
            {!backupHealthy && backupEnabled && (
              <AlertTriangle
                className="w-5 h-5 text-amber-500 animate-pulse"
                title={systemHealth.backupSource.message || "Offline"}
              />
            )}
      ...
      <section className="bg-app-card p-6 rounded-xl border border-app-border shadow-xl">
        ...
        <div className="max-w-md">
          <label className="block text-sm font-medium text-app-dim mb-2">
            Timezone (IANA)
          </label>
          <input
