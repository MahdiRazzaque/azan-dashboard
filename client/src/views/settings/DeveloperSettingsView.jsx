...
export default function DeveloperSettingsView() {
  ...
  const handleRunJob = async (jobName) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.message || data.error || "Failed to trigger job");
      setMessage({
        type: "success",
        text: `Job "${jobName}" executed successfully.`,
      });
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-app-text mb-1">
            Developer Tools
          </h2>
          <p className="text-app-dim text-sm">
            System diagnostics and maintenance operations.
          </p>
        </div>
      ...
      <div className="min-h-[400px]">
        ...
        {activeTab === "logs" && <SystemLogsTab logs={logs} />}
      </div>
    </div>
  );
}
