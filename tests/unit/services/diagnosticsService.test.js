...
describe('Diagnostics Service', () => {
    const mockConfig = {
        ...
        automation: {
            triggers: {
                fajr: {
                    adhan: { enabled: true, type: 'file', path: 'adhan.mp3', targets: ['local'] },
                    preAdhan: { enabled: true, type: 'tts', template: 'Pre-adhan', offsetMinutes: 5, targets: ['browser'] },
                    iqamah: { enabled: true, type: 'url', url: 'http://example.com/iqamah.mp3' },
    ...
    describe('getAutomationStatus', () => {
        ...
        it('should handle different trigger types and details', async () => {
            ...
            expect(result.fajr.iqamah.details.source).toBe('http://example.com/iqamah.mp3');
        });
        ...
        it('should handle iqamah override', async () => {
            ...
            expect(calculateIqamah).toHaveBeenCalledWith(mockPrayerData.prayers.dhuhr, configWithOverride.prayers.dhuhr, 'Europe/London');
        });
        ...
        it('should handle long TTS templates in source detail', async () => {
            prayerTimeService.getPrayerTimes.mockResolvedValue(mockPrayerData);
            const longTemplate = 'A very long template that exceeds thirty characters for testing purposes';
            const configLongTTS = {
                ...mockConfig,
                automation: {
                    triggers: {
                        fajr: { adhan: { enabled: true, type: 'tts', template: longTemplate } }
                    }
                }
            };
            const result = await service.getAutomationStatus(configLongTTS);
            expect(result.fajr.adhan.details.source).toBe('"' + longTemplate.substring(0, 30) + '..."');
        });
        ...
        it('should handle url type with missing url', async () => {
            ...
            const configNoUrl = {
                ...mockConfig,
                automation: { triggers: { fajr: { adhan: { enabled: true, type: 'url' } } } }
            };
        ...
        it('should handle tts type with missing template', async () => {
            ...
            const configNoTpl = {
                ...mockConfig,
                automation: { triggers: { fajr: { adhan: { enabled: true, type: 'tts' } } } }
            };
    ...
    describe('getTTSStatus', () => {
        ...
        it('should handle DISABLED triggers', async () => {
            const config = { automation: { triggers: { fajr: { adhan: { enabled: false } } } } };
        ...
        it('should handle URL type', async () => {
            const config = { automation: { triggers: { fajr: { adhan: { enabled: true, type: 'url', url: 'http://test' } } } } };
            const result = await service.getTTSStatus(config);
        ...
        it('should handle FILE type', async () => {
            const config = { automation: { triggers: { fajr: { adhan: { enabled: true, type: 'file', path: 'test.mp3' } } } } };
            const result = await service.getTTSStatus(config);
        ...
        it('should handle missing file path for FILE type', async () => {
            const config = { automation: { triggers: { fajr: { adhan: { enabled: true, type: 'file' } } } } };
            const result = await service.getTTSStatus(config);
            expect(result.fajr.adhan.detail).toBe('Unknown');
        });

        it('should handle TTS GENERATED status', async () => {
            const config = { automation: { triggers: { fajr: { adhan: { enabled: true, type: 'tts', template: 'Test' } } } } };
            fs.promises.access.mockResolvedValue();
            fs.promises.readFile.mockResolvedValue(JSON.stringify({ text: 'Test', generatedAt: 'now' }));
        ...
        it('should handle TTS MISMATCH status', async () => {
            const config = { automation: { triggers: { fajr: { adhan: { enabled: true, type: 'tts', template: 'New' } } } } };
            fs.promises.access.mockResolvedValue();
            fs.promises.readFile.mockResolvedValue(JSON.stringify({ text: 'Old', generatedAt: 'now' }));
        ...
        it('should handle UNKNOWN trigger type', async () => {
            const config = { automation: { triggers: { fajr: { adhan: { enabled: true, type: 'ghost' } } } } };
            const result = await service.getTTSStatus(config);
